...
if ($_REQUEST['save'] == 'true') {
	...
	if (isset($err)) {
		...
		if (isDocumentsError($res)) {
			...
			if (!empty($_REQUEST['success_url'])) {
				...
				$errorMessage = 'The document has been saved. <a href="javascript:window.close()">Fenster schlieﬂen</a>';
				$printForm = FALSE;
			}
		}
	}
	...
	if (!empty($_REQUEST['cancel_url'])) {
		...
		$errorMessage = 'The process has been canceled. <a href="javascript:window.close()">Fenster schlieﬂen</a>';
		$printForm = FALSE;
	}
}
