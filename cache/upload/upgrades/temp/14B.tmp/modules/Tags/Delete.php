...
{
  sugar_die(LBL_NO_DELETE_PRIV);
}

$tag->retrieve($_REQUEST['record']);
$tag->mark_deleted($_REQUEST['record']);

$rmodule = $_REQUEST['return_module'];
$raction = $_REQUEST['return_action'];
$rid     = $_REQUEST['return_id'];

header("Location: index.php?module=".$rmodule."&action=".$raction."&record=".$rid);
