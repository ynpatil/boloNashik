...
class BirthdaylistDashlet extends DashletGenericBDL { 
    ...
    function BirthdaylistDashlet($id, $def = null) {
        ...
        $this->columns = $dashletData['BirthdaylistDashlet']['columns'];
    ...
    function getTitle($text) {
        ...
        $str .= 'id="dashlet_header_' . $this->id . '">' . "<form method='POST' action='index.php'>\n".
			...
			"<input type='hidden' name='action' value='index'>\n".get_form_header($this->title, $this->dashletStrings['LBL_BIRTHDAYLIST_FORM_TITLE'].$this->day_filter.' ('.$timedate->to_display_date($this->later, false).') </td><td>'.$additionalTitle, false) ."</form>". '</div>';
        return $str;
    }
