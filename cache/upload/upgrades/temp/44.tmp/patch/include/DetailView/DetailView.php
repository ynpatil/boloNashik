...
class DetailView extends ListView {
	...
	function processSugarBean($html_varName, $seed, &$offset, $isfirstview=0) { 
        ...
        $this->query_orderby =  $seed->process_order_by($this->getLocalSessionVariable($html_varName, "ORDER_BY_DETAIL"),null);
	...
	function processListNavigation( &$xtpl, $html_varName, $current_offset, $display_audit_link = false){
        ...
        if($display_audit_link && (!isset($sugar_config['disc_client']) || $sugar_config['disc_client'] == false))
        {
            ...
            $audit_link = "<a href='#' onclick='open_popup(\"Audit\", \"600\", \"400\", \"&record=".$_REQUEST['record']."&module_name=".$_REQUEST['module']."\", true, false, $encoded_popup_request_data);' class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_VIEW_CHANGE_LOG']."</a>";
        }
        ...
        if ($this->return_to_list_only == true) {
	        if($current_offset != 0 && $this->isRequestFromListView($html_varName))
	        {
    			...
    			$list_link = "<a href=\"$list_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_RETURN']."&nbsp;</a>";

				$html_text .= "<td nowrap align='right' class='tabDetailViewDL'>".$list_link;
			
				if ($row_count != 0) {
		    		$resume_URL  = $this->base_URL.$current_offset."&InDetailNav=1";
	    			$resume_link = "<a href=\"$resume_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_RESUME']."&nbsp;</a>";
	        ...
	        if($current_offset != 0 && $this->isRequestFromListView($html_varName))
	        {
	    		...
	    		if(1 == $current_offset){
	    			$start_link = get_image($image_path."start_off","alt='".$this->local_app_strings['LNK_LIST_START']."'  border='0' align='absmiddle'")."&nbsp;".$this->local_app_strings['LNK_LIST_START'];
	   				...
	   				$start_link = "<a href=\"$start_URL\" class=\"listViewPaginationLinkS1\">".get_image($image_path."start","alt='".$this->local_app_strings['LNK_LIST_START']."'  border='0' align='absmiddle'")."</a>&nbsp;<a href=\"$start_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_START']."</a>";
	   				if(0 != $current_offset){
		   				$previous_link = "<a href=\"$previous_URL\" class=\"listViewPaginationLinkS1\">".get_image($image_path."previous","alt='".$this->local_app_strings['LNK_LIST_PREVIOUS']."'  border='0' align='absmiddle'")."</a>&nbsp;<a href=\"$previous_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_PREVIOUS']."</a>";
	    			}
	    			else {
	    				$previous_link = get_image($image_path."previous_off","alt='".$this->local_app_strings['LNK_LIST_PREVIOUS']."'  border='0' align='absmiddle'")."&nbsp;".$this->local_app_strings['LNK_LIST_PREVIOUS'];
	    			}
	    		...
	    		if($row_count <= $current_offset){
	    			...
	    			$end_link = "<a href=\"$end_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_END']."</a>&nbsp;<a href=\"$end_URL\" class=\"listViewPaginationLinkS1\">".get_image($image_path."end","alt='".$this->local_app_strings['LNK_LIST_END']."'  border='0' align='absmiddle'")."</a>";
	    			$next_link = "<a href=\"$next_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_NEXT']."</a>&nbsp;<a href=\"$next_URL\" class=\"listViewPaginationLinkS1\">".get_image($image_path."next","alt='".$this->local_app_strings['LNK_LIST_NEXT']."'  border='0' align='absmiddle'")."</a>";
	    		}
	
	    		$list_link = "<a href=\"$list_URL\" class=\"listViewPaginationLinkS1\">".$this->local_app_strings['LNK_LIST_RETURN']."&nbsp;</a>";
	
	   			$html_text .= "<td nowrap align='right' class='listViewPaginationTdS1'>".$list_link."&nbsp;&nbsp;&nbsp;&nbsp;".$start_link."&nbsp;&nbsp;".$previous_link."&nbsp;&nbsp;(".$current_offset." ".$this->local_app_strings['LBL_LIST_OF']." ".$row_count.")&nbsp;&nbsp;".$next_link."&nbsp;&nbsp;".$end_link."</td>";
...
/* BEGIN - ORGANIZATIONAL CHART - INCLUDED by MR. MILK, 2005-03-20 */
		global $app_strings;
		if(file_exists("orgchart.php") && strpos("Accounts#Contacts#Users#Employees", $_REQUEST['module'])!==false) {
			$xtpl->assign("ORGCHART", '<input LANGUAGE=javascript class="button" value="'.$app_strings['LBL_ORGCHART_BUTTON_TITLE'].'" type="button" onclick='."'".'status="";return window.open("orgchart.php?&module='. $_REQUEST['module'] . '&record='. $_REQUEST['record'] . '", "orgchart", "width=640, height=480,resizable=1,scrollbars=1");'. "'" . '>');
		}
/* END - ORGANIZATIONAL CHART - INCLUDED by MR. MILK, 2005-03-20 */

/* BEGIN - GANTT CHART - INCLUDED by MR. MILK, 2005-03-20 
 *		global $app_strings;
 *		if(file_exists("ganttchart.php") && strpos("Project#ProjectTask", $_REQUEST['module'])!==false) {
 *			$xtpl->assign("GANTTCHART", '<input LANGUAGE=javascript class="button" value="'.$app_strings['LBL_GANTT_BUTTON_TITLE']   .'" type="button" onclick='."'".'status="";return window.open("ganttchart.php?date=d/m&module='. $_REQUEST['module'] . '&record='. $_REQUEST['record'] . '", "ganttchart", "width=640, height=480,resizable=1,scrollbars=1");'. "'" . '>');
 *		}
 * END - GANTT CHART - INCLUDED by MR. MILK, 2005-03-20 */

/* BEGIN - DOCUMENT MERGE - INCLUDED by MR. MILK, 2005-03-20 
 *		if(file_exists("modules/DocMerge/MailMerge.php")) {
 *			$docmerge =  '<input type="hidden" name="merge_module">'
 *						.'<input type="hidden" name="merge_action">'
 *						.'<input type="hidden" name="merge_id">';
 *			$xtpl->assign("VARMERGE", $docmerge);
 *		 	$xtpl->assign("DOCMERGE", '<input type="button" class="button" value="'.$app_strings['LBL_DOCMERGE_BUTTON_TITLE'].'" name="button" LANGUAGE=javascript onclick='."'".'window.open("index.php?module=DocMerge&action=Popup&form=DetailView&form_submit=true&query=true","new","width=640, height=480,resizable=1,scrollbars=1");'. "'" . '>');
 *		}
 * END - DOCUMENT MERGE - INCLUDED by MR. MILK, 2005-03-20 */
