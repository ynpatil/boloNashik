			...
			while ($i < $rows)
			{
				...
				if($circ_ref != 'Skip Done')
				{
					...
					if($row['type'] != 'Contacts' || !$security || $current_team->can_access_record($row['id'], 'detailview')=='')
						$link .= "<a status='' title='".$row['tooltip']."' style='text-decoration: none' href='' onClick=".'"'."JavaScript:window.opener.location='index.php?action=DetailView&module=".$row['type']."&record=".$row['id']."';window.close();return false;".'"'."'>$circ_ref ".$row['name']."</a>";
					else
