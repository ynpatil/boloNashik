...
#debug		echo "type=$type id=$id  \n";
		...
		if($first)
		{
			...
			{
				$sql  = "SELECT ";
				$sql .= "  reports_to_id, id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip,function_mast.name as function_name,'Contacts' as type, assigned_user_id, '' as acc_id ";
				$sql .= "FROM ";
		...
		else
		{
			...
			{
				$sql  = "SELECT ";
				$sql .= "  reports_to_id, contacts.id as id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip, function_mast.name as function_name, 'Contacts' as type, assigned_user_id, account_id as acc_id ";
				$sql .= "FROM ";
				$sql .= "  contacts LEFT JOIN function_mast ON contacts.function_id = function_mast.id ";
				$sql .= "LEFT JOIN ";
				$sql .= "  accounts_contacts ";
...
#notitle					$link .= "<a status='' title='" . $row['tooltip'] . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name']."</a>";
						$link .= "<a status='' title='" . $row['tooltip'] . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name'] . "<BR><BR>" . $row['tooltip'] . "<BR><BR>". $row['function_name']."</a>";
					else
					...
					$GLOBALS['log']->debug("Link ref ".$link);					
...
EOQ;
	...
	echo "<!-- Source file -->\n";
	echo "<script src=\"include/javascript/yui/dom-0.10.0.js\"></script>\n";
