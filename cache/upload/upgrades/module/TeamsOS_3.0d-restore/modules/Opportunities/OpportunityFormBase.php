...
// $Id: OpportunityFormBase.php,v 1.64 2006/06/06 17:58:22 majed Exp $
...
<tr>
    <td class="dataLabel">$lbl_amount&nbsp;<span class="required">$lbl_required_symbol</span></td>
</tr>
<tr>
    <td class="dataField"><input name='{$prefix}amount' type="text" value='{$opp_amount}'></td>
</tr>
EOQ;
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if($focus->sales_stage == "Closed Won" || $focus->sales_stage == "Closed Lost"){
		...
		if(count($data) == 0){			
			...
			$focusReview->assigned_user_id = (isset($current_user->reports_to_id)?$current_user->reports_to_id:$current_user->id);
	...
	///
	$GLOBALS['log']->debug("Saved record with opportunity.id of ".$return_id);
