...
{
    //Header
    $size = array(20,40,40,30,30,30,30);
    $i = 0;
    ...
    foreach($data as $row)
    {
        $this->Cell($size[0],6,$row['BRANCH'],'LR',0,'L',$fill);
        ...
        $this->Cell($size[6],6,$row['AVERAGE_FIELD_TIME'],'LR',0,'L',$fill);
        $this->Cell($size[7],6,$row['TOTAL_COUNT'],'LR',0,'L',$fill);
        $fill = !$fill;
		$rowcount++;            
        $this->Ln();
    }
