 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.3 2006/06/06 17:58:55 majed Exp $
...
$home_spacer = 'table_main_home';
}
else $home_spacer = 'table_main';

$xtpl->assign("HOME_SPACER", $home_spacer);
...
foreach($preListHeader as $module_name)
{
		...
		if ($i==$max_tabs-1 and $numb_tabs>$max_tabs) {
			$xtpl->assign("MORE", "<a href='javascript:void(0);' id='MoreHandle' cursor: pointer; cursor: hand;' onmouseover='tbButtonMouseOver(this.id,75,\"\",0);'>more <b>�</b></a>");
		} 
		$xtpl->parse("main.tab");
	$i++;
}
...
foreach($module_menu as $menu_item)
{
	...
	$xtpl->parse("main.left_form.sub_menu.hide_sub_menu_item");
}
}
...
$xtpl->assign("LEFT_FORM_OTD", '<img style="cursor: pointer; cursor: hand; margin-top: 28px;" id="HideHandle" name="HideHandle" src="'.$image_path.'show.gif" alt="" onclick=\'hideLeftCol("leftCol");closeMenus();\' onmouseover="showSubMenu(\'leftCol\')">');
$xtpl->assign("LEFT_FORM_CTD", '</div></td>');
...
if (count($history) > 0) {
	...
	if ($current_row < 9) {
		...
		$xtpl->parse("main.hide_recent_view.hide_recent_view_row");
	}
		$current_row++;
	}
}
else {
		$xtpl->parse("main.hide_recent_view.hide_recent_view_empty");
		$xtpl->parse("main.left_form_recent_view.left_form_recent_view_empty");
}
$xtpl->parse("main.hide_recent_view");
$xtpl->parse("main.left_form_recent_view");


require_once("modules/".$currentModule."/Forms.php");
if ($currentModule && $action == "index" && function_exists('get_new_record_form') && get_new_record_form() !="") {
