 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.10 2006/08/03 18:49:18 jbenterou Exp $
		...
		if($linkattribute == 'linkinfo') {
			foreach ($attributevalue as $label => $url) {
				...
				if($i < sizeof($global_control_links)-1) {
					$xtpl->assign("SEPARATOR", "<img src='".$image_path."myAreaSeparator.gif' width='2' height='12' border='0' align='absmiddle' style='margin: 1px 4px 1px 6px'>");
				} else {
	...
	foreach($pre_module_menu as $menu_item)
	{
		...
		if(!isset($menu_item[3])|| !isset($modListHeader) || (isset($menu_item[3]) && (key_exists($menu_item[3],$modListHeader) || $subModuleCheck))){
			...
			if ($menu_item[1] != 'Deleted Items') {
				...
				$xtpl->assign("SC_IMAGE", get_image($image_path.$menu_item[2],"alt='".$menu_item[1]."'  border='0' align='middle'"));
				if ($i==$max_sub_tabs-1 and $numb_tabs>$max_sub_tabs) {
					$xtpl->assign("SEPARATOR", "<img src='".$image_path."shortcutSeparator.gif' width='2' height='16' border='0' align='middle' style='margin-left: 10px; margin-right: 6px;'><img src='".$image_path."moreSub.gif' width='18' height='18' border='0' align='middle' onmouseover='tbButtonMouseOver(this.id,90,\"\",\"0\");' id='SubMoreHandle'>");
				} else {
					if ($i!=$numb_tabs-1) {
						$xtpl->assign("SEPARATOR", "<img src='".$image_path."shortcutSeparator.gif' width='2' height='16' border='0' align='middle' style='margin-left: 8px; margin-right: 4px;'>");
					} else {
						$xtpl->assign("SEPARATOR", "");
					}
				}
