 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.10 2006/06/06 17:58:55 majed Exp $
		...
		if($linkattribute == 'linkinfo') {
			foreach ($attributevalue as $label => $url) {
				...
				if (isset($sub_menu[$key]) && $sub_menu[$key]) {
					$xtpl->assign("GCL_MENU", "id='".$key."Handle' onmouseover=' tbButtonMouseOver(this.id,89,\"\",0);'");
					$xtpl->assign("MENU_ARROW", "<img src='".$image_path."menuarrow.gif' alt=''  id='".$key."Handle' style='margin-bottom: 1px; margin-left:2px; cursor: pointer; cursor: hand;' valign='absmiddle' onmouseover='tbButtonMouseOver(this.id,89,\"\",0);'>");
				} else {
...
foreach($preListHeader as $module_name)
{
		...
		if ($i==$max_tabs-1 and $numb_tabs>$max_tabs) {
			$xtpl->assign("MORE", "<img src='".$image_path."more.gif' alt='' align='absmiddle' id='MoreHandle' style='margin-bottom: 1px; margin-left:2px; cursor: pointer; cursor: hand;' align='absmiddle' onmouseover='tbButtonMouseOver(this.id,69,\"\",0);'>");
		} 
		$xtpl->parse("main.tab");
	$i++;
}
