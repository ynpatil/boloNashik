 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.8 2006/06/06 17:58:55 majed Exp $
		...
		if($linkattribute == 'linkinfo') {
			foreach ($attributevalue as $label => $url) {
				...
				if (isset($sub_menu[$key]) && $sub_menu[$key]) {
					...
					$xtpl->assign("MENU_ARROW", "<img src='".$image_path."menuarrow.gif' alt='' align='absmiddle' id='".$key."Handle' style='margin-bottom: 1px; margin-left:2px; cursor: hand;' align='absmiddle' onmouseover='tbButtonMouseOver(this.id,18,\"\",0);'>");
				} else {
					$xtpl->assign("GCL_MENU", "");
					$xtpl->assign("MENU_ARROW", "");
				}
				if($i < sizeof($global_control_links)-1) {
					$xtpl->assign("SEPARATOR", "&nbsp;<img src='".$image_path."myAreaSeparator.gif' alt='|' align='absmiddle'>&nbsp;");
				} else {
...
else $xtpl->assign("ONLOAD", 'onload="ss_timer();"');
...
foreach($preListHeader as $module_name)
{
		...
		if ($i==$max_tabs-1 and $numb_tabs>$max_tabs) {
			$xtpl->assign("MORE", "<img src='".$image_path."more.gif' alt='' align='absmiddle' id='MoreHandle' style='margin-bottom: 1px; margin-left:2px; cursor: hand;' align='absmiddle' onmouseover='tbButtonMouseOver(this.id,68,\"\",0);'>");
		} 
		$xtpl->parse("main.tab");
	$i++;
}
