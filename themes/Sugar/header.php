 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: header.php,v 1.71 2006/08/27 19:46:19 jbenterou Exp $
...
$xtpl->assign('PASSWORD',$_SESSION['login_c_password']);
$xtpl->assign('USERNAME',$_SESSION['login_c_user_name']);
$xtpl->assign('RESPOSITORY_LOGIN_URL',RESPOSITORY_LOGIN_URL);
$xtpl->assign('APPROVAL_LOGIN_URL',APPROVAL_LOGIN_URL);
...
foreach($global_control_links as $key => $value) {
	...
	$xtpl->parse("main.hide_top_menus.global_control_links");
	$i++;
}
...
if(($action=='Login' || $action=='Register' || $action=='location') && $sugar_config['login_nav']==false)
{
    $modListHeader = array();
}
else
...
$activities= array("Calls","Meetings","Tasks","Notes","Reviews");
...
if(!empty($theme_colors))
{
    ...
    foreach($theme_colors as $color)
    {
		...
		$xtpl->parse("main.hide_top_menus.color");
...
if(!empty($theme_fonts))
{
    ...
    foreach($theme_fonts as $font)
    {
        ...
        $xtpl->parse("main.hide_top_menus.font");
...
if($action!='location'){
    $xtpl->parse("main.hide_top_menus");
}
...
{
	$subModuleCheck = 0;
	$subModuleCheckArray = array("Tasks", "Calls", "Meetings", "Notes","Prospects","Reviews");
	...
	if(!isset($menu_item[3])|| !isset($modListHeader) || (isset($menu_item[3]) && (key_exists($menu_item[3],$modListHeader) || $subModuleCheck)))
    {
        ...
        {
    		if(strstr($menu_item[1],'Mobile')){
    			$split_menu = explode("/",$menu_item[1]);
    			$xtpl->assign("URL", $menu_item[0]);
    			$xtpl->assign("LABEL", $split_menu[0]);

    			$xtpl->assign("URL1", $menu_item[0]."&action=RequestsListView");
    			$xtpl->assign("LABEL1", get_image($image_path.'mobile',"alt='mForce : ".$menu_item[1]."'  border='0' align='absmiddle'").$split_menu[1]);
    		}
    		else{
    			$xtpl->assign("URL", $menu_item[0]);
