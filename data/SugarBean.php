    ...
    function SugarBean() {
        ...
        if (empty($loaded_defs[$this->object_name])) {
            ...
            if (empty($this->skip_fields))
                $this->skip_fields = LoadCachedArray($this->module_dir, $this->object_name, 'skip_fields'); //jaiganesh
        ...
        if (( $this->object_name == 'Meeting' || $this->object_name == 'Call' ) || $notify_user->receive_notifications) {
            if (empty($notify_user->email1) && empty($notify_user->email2)) {
                ...
                if (!$notify_mail->Send()) {
                    ...
                    $GLOBALS['log']->info("Notifications: e-mail successfully sent " . $notify_mail->From);
                }
            }
        }
    }
...
//runs into an issue when populating from field_defs for users - corrupts user prefs
    ...
    function get_related_list($child_seed, $related_field_name, $order_by = "", $where = "", $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0) {
...
//		$GLOBALS['log']->debug("get_full_list:  order_by = '$order_by' and where = '$where'");
    ...
    function create_new_list_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0, $join_type = '', $return_array = false, $parentbean, $singleSelect = false) {
        ...
        } else if ($this->bean_implements('ACL') && ACLController::requireOwnerOrCreator($this->module_dir, 'list')) {
            global $current_user;
        ...
        } else if ($this->bean_implements('ACL') && ACLController::requireMyTeam($this->module_dir, 'list')) {
            global $current_user;
        ...
        } else if ($this->bean_implements('ACL') && ACLController::requireMyLeadTeam($this->module_dir, 'list')) {
            global $current_user;
            $GLOBALS['log']->debug("SugarBean:create_new_list_query.ParentBean Name" . get_class($parentbean));
            $GLOBALS['log']->debug("SugarBean:create_new_list_query.Bean Module Name" . $parentbean->module);
            $GLOBALS['log']->debug("SugarBean:create_new_list_query.Bean Name" . get_class($this));
            $GLOBALS['log']->debug("SugarBean:create_new_list_query.Module Name" . $this->object_name);
            $owner_where = getMyLeadTeamFromClause($this->record, strtolower($this->object_name), $this->table_name, $this->table_name, $this->field_defs);
            if ($owner_where) {
                $where.=" and " . $owner_where;
            }
            $GLOBALS['log']->debug("SugarBean:create_new_list_query[getMyLeadTeamFrom clause] :" . $owner_where);
        }

        $GLOBALS['log']->debug("SugarBean:create_new_list_query :request module " . $_REQUEST['module']);
        #if($this->bean_implements('ACL') && ACLController::requireMyDealTeam($this->module_dir, 'list') && ($this->object_name=='Opportunity' && trim($_REQUEST['module'])=='Opportunities') || ($this->object_name=='Deal' && $_REQUEST['module']=='Deals') ) {
        
       //echo "HERERE >>>>>>>>>>>>>". ACLController::requireMyVendorTeam($this->module_dir, 'list');
        // MY Team Vendor
        if ($this->bean_implements('ACL') && ACLController::requireMyVendorTeam($this->module_dir, 'list')) {
            
            global $current_user;
            $GLOBALS['log']->debug("SugarBean:requireMyVendorTeam:create_new_list_query.ParentBean Name" . get_class($parentbean));
            $GLOBALS['log']->debug("SugarBean:requireMyVendorTeam:create_new_list_query.Bean Module Name" . $parentbean->module);
            $GLOBALS['log']->debug("SugarBean:requireMyVendorTeam:create_new_list_query.Bean Name" . get_class($this));
            $GLOBALS['log']->debug("SugarBean:requireMyVendorTeam:create_new_list_query.Module Name" . $this->object_name);
            //if($_REQUEST['action']!='DetailView'){
            $owner_where = getMyVendorTeamFromClause(&$this,$this->record, strtolower($this->object_name), $this->table_name, $this->table_name, $this->field_defs,$params);
           // }
            if ($owner_where) {
                $where.=" and " . $owner_where;
            }
            $GLOBALS['log']->debug("SugarBean:create_new_list_query[getMyVendorTeamFromClause ] :" . $owner_where);
        }
        //   echo "<br>>>>>>".$where;
...
//							$GLOBALS['log']->debug("$temp->object_name({$row['id']}): ".$field." = ".$temp->$field);
    ...
    function process_union_list_query($parent_bean, $query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $subpanel_def, $query_row_count = '', $secondary_queries = array()) {
        /**
    ...
    function process_detail_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $offset = 0) {
        ...
        // Check to see if we have a count query available.
        $MODULE_NAME = $_SESSION['MAILMERGE_MODULE_FROM_LISTVIEW'];
        if (empty($_SESSION['row_count'][$MODULE_NAME])) {
            $count_query = $this->create_list_count_query($query);
            ...
            if (!empty($count_query) && (empty($limit) || $limit == -1)) {
                ...
                if (!empty($assoc['c'])) {
                    $total_rows = $assoc['c'];
                    $_SESSION['row_count'][$MODULE_NAME] = $total_rows;
                }
            }
        } else {
            $total_rows = $_SESSION['row_count'][$MODULE_NAME];
        }
...
//		echo "Owner :".$is_owner;
        ...
        switch ($view) {
            ...
            case 'index':
            case 'ListView':
                //echo "<br>ListView>>>>>>>".ACLController::checkAccess($this->module_dir, 'list', true);
    ...
    function create_qualified_order_by($order_by, $qualify) { // if the column is empty, but the sort order is defined, the value will throw an error, so do not proceed if no order by is given
        if (empty($order_by)) {
            return $order_by;
        }
