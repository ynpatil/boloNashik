...
class SugarWidgetSubPanelTopSelectFromSAPAccountButton extends SugarWidgetSubPanelTopButton
{
...
//		echo "OM Button ".$this->module_name;		
		...
		$return_module = $focus->module_dir;//$_REQUEST['module'];
		$return_action = 'SubPanelViewer';
		$return_id = $focus->id;//$_REQUEST['record']; 
...
//		$initial_filter .= "&return_id=".$focus->id;
//		$initial_filter .= "&return_module=".$focus->module_dir;
...
//		echo "ACL Value :".ACLController::requireOwner('Accounts', 'edit');
		...
		return '<form action="index.php">' . "\n"		
			...
			. ' value="' . $this->value . "\"\n"
			. (!ACLController::checkAccess('Accounts', 'edit', true)?' disabled ':'  ')
			. " onclick='open_popup1(\"$this->module_name\",800,600,\"$initial_filter\",true,true,$json_encoded_php_array,\"$popup_mode\",$create);' /></form>\n";
	}
}
?>
