...
class SugarWidgetSubPanelTopSelectFromMemberAccountButton extends SugarWidgetSubPanelTopButton
{
	//button_properties is a collection of properties associated with the widget_class definition. layoutmanager
	function SugarWidgetSubPanelTopSelectFromMemberAccountButton($button_properties=array())
	{
...
//		echo "OM Button ".$this->module_name;		
		...
		$popup_request_data = array(
			'call_back_function' => 'set_return_to_linkage_screen',
...
//		$GLOBALS['log']->debug("Encoded php array ".$json_encoded_php_array);
		...
		return '<form action="index.php">' . "\n"
			...
			. ' value="' . $this->value . "\"\n"
			. " onclick='open_popup_for_member_accounts(\"$this->module_name\",600,400,\"$initial_filter\",true,true,$json_encoded_php_array,\"$popup_mode\",$create);' /></form>\n";
	}
}
?>
