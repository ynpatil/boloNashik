...
// $Id: sugar_3.js,v 1.124 2006/08/29 22:49:58 jbenterou Exp $
...
function check_form(formname) {
	...
	var returnVal = validate_form(formname, '');
	//alert("In check_form "+formname+" return val :"+returnVal);
	return returnVal;
}
	...
	for(var i = 0; i < validate[formname].length; i++){
			if(validate[formname][i][nameIndex].indexOf(startsWith) == 0){
				if(typeof eval(form + "['" + validate[formname][i][nameIndex] + "']" ) != 'undefined'){
					var bail = false;
					//alert("Some more :"+validate[formname][i][nameIndex]+" type "+(typeof eval(form + "['" + validate[formname][i][nameIndex] + "']"))+" required :"+validate[formname][i][requiredIndex]+" value :"+trim(eval(form + "['" + validate[formname][i][nameIndex] + "']" + ".value")));
					...
					if(!bail){
						//alert("Type switch :"+validate[formname][i][typeIndex]+" Error "+isError);
...
function sendAndRetrieve1(loadingStr) {
//	alert("In sendAdnRetrieve");
	if(typeof loadingStr == 'undefined') SUGAR.language.get('app_strings', 'LBL_LOADING');
	ajaxStatus.showStatus(loadingStr);
	return false;
		...
		getAdditionalDetails: function(bean, id, spanId) {
			go = function() {
				...
				success = function(data) {
					//alert("Before :"+data.responseText);
					var result = eval(data.responseText);
					//alert("Done eval :"+result['caption']);
			...
			SUGAR.util.additionalDetailsRpcCall = window.setTimeout('go()', 250);
                ...
                getAdditionalDetailsMeetingFeedback: function(bean, id, spanId) {
				...
				if(typeof SUGAR.util.additionalDetailsCalls[spanId] != 'undefined') // call already in progress
					return;
				ajaxStatus.showStatus(SUGAR.language.get('app_strings', 'LBL_LOADING'));
				url = 'index.php?to_pdf=1&module=Home&action=AdditionalDetailsMeetingFeedbackRetrieve&bean=' + bean + '&id=' + id;
				//alert("URL :"+url);
				SUGAR.util.additionalDetailsCalls[spanId] = YAHOO.util.Connect.asyncRequest('GET', url, {success: success, failure: success});

				return false;
			}
			SUGAR.util.additionalDetailsRpcCall = window.setTimeout('go()', 150);
		},
		getAdditionalDetailsActivityReport: function(bean, id, spanId) {
			go = function() {
				oReturn = function(body, caption, width, theme) {
					return overlib(body, CAPTION, caption, STICKY, MOUSEOFF, 1000, WIDTH, width, CLOSETEXT, ('<img border=0 src=themes/' + theme + '/images/close_inline.gif>'), CLOSETITLE, 'Click to Close', CLOSECLICK, FGCLASS, 'olFgClass', CGCLASS, 'olCgClass', BGCLASS, 'olBgClass', TEXTFONTCLASS, 'olFontClass', CAPTIONFONTCLASS, 'olCapFontClass', CLOSEFONTCLASS, 'olCloseFontClass', REF, spanId, REFC, 'LL', REFX, 13);
				}
				...
				if(typeof SUGAR.util.additionalDetailsCalls[spanId] != 'undefined') // call already in progress
					return;
				ajaxStatus.showStatus(SUGAR.language.get('app_strings', 'LBL_LOADING'));
				url = 'index.php?to_pdf=1&module=Home&action=AdditionalDetailsActivityReportRetrieve&bean=' + bean + '&id=' + id;
				//alert("URL :"+url);
				SUGAR.util.additionalDetailsCalls[spanId] = YAHOO.util.Connect.asyncRequest('GET', url, {success: success, failure: success});
