...
// $Id: dashlets.js,v 1.4 2006/08/22 21:58:01 awu Exp $
...
SUGAR.dashlets = function() {
	return {
		...
		postFormWithMethodParams: function(theForm, callback) {	
			var success = function(data) {
				if(data) {
					var form = (document.getElementById(theForm) || document.forms[theForm]);
					//alert("form :"+form+" callback "+callback);
					callback(data.responseText,form.record.value);
				}
			}
			YAHOO.util.Connect.setForm(theForm); 
			var cObj = YAHOO.util.Connect.asyncRequest('POST', 'index.php', {success: success, failure: success});
