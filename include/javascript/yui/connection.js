...
Line 595 WP: pass in empty values for option select boxes instead of passing in the text
*/
					...
					case 'select-multiple':
						for(var j=0; j<oElement.options.length; j++){
							if(oElement.options[j].selected){
								// SugarCRM (wp 8-11-2006): if the value is empty on an option, pass in an empty value
								if(typeof oElement.options[j].value == 'undefined') 
									value = oElement.options[j].text;
								else 
									value = oElement.options[j].value;
								this._sFormData += encodeURIComponent(oName) + '=' + encodeURIComponent(value) + '&';
								// the following line, for the selected <option value=''>--None--</option>
								// would pass --None--, instead of ''
