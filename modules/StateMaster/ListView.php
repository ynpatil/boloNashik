...
if(file_exists('custom/modules/StateMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/StateMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/StateMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'StateMaster');
...
$seedState = new State(); // seed bean
$searchForm = new SearchForm('StateMaster', $seedState); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['StateMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['StateMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['StateMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "StateMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedState, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('StateMaster');
