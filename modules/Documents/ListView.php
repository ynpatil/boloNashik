 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.32 2006/09/06 21:39:19 jenny Exp $
...
if(file_exists('custom/modules/Documents/metadata/listviewdefs.php')){
	require_once('custom/modules/Documents/metadata/listviewdefs.php');	
}else{
	require_once('modules/Documents/metadata/listviewdefs.php');
}
...
$seedDocument = new Document(); // seed bean
$searchForm = new SearchForm('Documents', $seedDocument); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Documents'][$col])) 
            $displayColumns[$col] = $listViewDefs['Documents'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Documents'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Documents"); // builds the where clause from search field inputs
	if (count($where_clauses) > 0 )$where = implode(' and ', $where_clauses);    
    $GLOBALS['log']->info("Here is the where clause for the list view: $where");
}

//jai ganesh for showing only Private,MyTeam & Global records of the current user
if(!is_admin($current_user)){
		...
		$additionalWhere = " (documents.document_type = 'Global') OR ( 
...
$lv->setup($seedDocument, 'include/ListView/ListViewGeneric.tpl', $where, $params);
