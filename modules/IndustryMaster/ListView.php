...
if(file_exists('custom/modules/IndustryMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/IndustryMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/IndustryMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'IndustryMaster');
...
$seedIndustry = new Industry(); // seed bean
$searchForm = new SearchForm('IndustryMaster', $seedIndustry); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['IndustryMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['IndustryMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['IndustryMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "IndustryMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedIndustry, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('IndustryMaster');
