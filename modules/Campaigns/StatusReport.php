...
echo "\n</p>\n";
$xtpl = new XTemplate('modules/Campaigns/StatusReport.html');
...
foreach ($vendor_ids as $key => $vendor_id) {
    $VendorObj->retrieve($vendor_id);
    $VendorArr[$VendorObj->id] = $VendorObj->name;
    
}
$xtpl->assign("VENDOR_NAME", get_select_options_with_id($VendorArr, $_REQUEST['vendor_id']));
$xtpl->assign("CALL_STATUS", get_select_options_with_id($app_list_strings['call_status_dom'], $_REQUEST['call_status']));


if($_REQUEST['FileLink']){
    $xtpl->assign("FILE_LINK", $_REQUEST['FileLink']);
    $xtpl->assign("DOWNLOAD_LINK", "Download File:");
    $xtpl->assign("DOWNLOAD_NAME", "Click Here");
}
