...
if(file_exists('custom/modules/Campaigns/metadata/listviewdefs.php')){
	require_once('custom/modules/Campaigns/metadata/listviewdefs.php');	
}else{
	require_once('modules/Campaigns/metadata/listviewdefs.php');
}
...
$seedCampaign = new Campaign(); // seed bean
$searchForm = new SearchForm('Campaigns', $seedCampaign); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Campaigns'][$col])) 
            $displayColumns[$col] = $listViewDefs['Campaigns'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Campaigns'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Campaigns"); // builds the where clause from search field inputs
...
$lv->setup($seedCampaign, 'include/ListView/ListViewGeneric.tpl', $where, $params, 0, -1, array_keys($displayColumns));
