...
require_once ('modules/RegionMaster/Region.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/RegionMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/RegionMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/RegionMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'RegionMaster');
...
$seedBranch = new Region(); // seed bean
$searchForm = new SearchForm('RegionMaster', $seedBranch); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['RegionMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['RegionMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['RegionMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "RegionMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedBranch, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('RegionMaster');
