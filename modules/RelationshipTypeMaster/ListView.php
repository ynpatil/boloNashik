 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.22.4.1 2006/09/13 00:50:39 jenny Exp $
...
require_once ('modules/RelationshipTypeMaster/RelationshipType.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/RelationshipTypeMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/RelationshipTypeMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/RelationshipTypeMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'RelationshipTypeMaster');
...
$seedRelationshipType = new RelationshipType(); // seed bean
$searchForm = new SearchForm('RelationshipTypeMaster', $seedRelationshipType); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['RelationshipTypeMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['RelationshipTypeMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['RelationshipTypeMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "RelationshipTypeMaster"); // builds the where clause from search field inputs
...
echo "<br/><a class=\"listViewTdLinkS1\" href='index.php?module=$currentModule&return_action=ListView&return_module=$currentModule&action=EditView'>Create new record</a>";

$lv->setup($seedRelationshipType, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('RelationshipTypeMaster');
//$GLOBALS['log']->debug("Saved search result :".$savedSearchResult);

$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearchResult));
$str = "<script>
