 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: EditView.php,v 1.26 2006/07/31 21:02:19 jenny Exp $
...
}if (isset($_REQUEST['campaign_id'])) $xtpl->assign("CAMPAIGN_ID", $_REQUEST['campaign_id']);
...
}elseif (is_null($focus->parent_type) || empty($focus->parent_type)) {
	$focus->parent_type = $app_list_strings['prospect_list_default_key'];
}

$xtpl->assign("TYPE_OPTIONS", get_select_options_with_id($app_list_strings['prospect_list_type_dom'], $focus->parent_type));
$parent_types = $app_list_strings['prospect_list_type_dom'];
$disabled_parent_types = ACLController::disabledModuleList($parent_types,false, 'list');

//print_r($disabled_parent_types);exit;
...
function changeQS() {
	new_module = document.EditView.parent_type.value;
        ...
        if(new_module == 'CityMaster' || new_module == 'Leads' || typeof(disabledModules[new_module]) != 'undefined') {
...
/*
$xtpl->assign("LIST_TYPE_VALUE", $focus->list_type_value);
if ($focus->list_type == 'level') {
    $level=$focus->list_type_value;
} else if ($focus->list_type == 'experience') {
    $experience=$focus->list_type_value;
} else if ($focus->list_type == 'city') {
    $city=$focus->list_type_value;
}
// LEVEL 
    $xtpl->assign("LEVEL", get_select_options_with_id($app_list_strings['level_dom'], $level)); //ynp
//Experience
    for ($e = 0; $e <= 50; $e++) {
        $exp_arr[$e] = $e;
    }
    $xtpl->assign("EXPERIENCE", get_select_options_with_id($exp_arr, $experience));
//City
    $city_arr=get_city_array();
    $xtpl->assign("CITY", get_select_options_with_id($city_arr, $city));

if (empty($focus->list_type) or $focus->list_type!='exempt_domain') {
	$xtpl->assign("DOMAIN_HIDDEN", "display:none");
}
if (empty($focus->list_type) or $focus->list_type!='level') {
	$xtpl->assign("LEVEL_HIDDEN", "display:none");
}
if (empty($focus->list_type) or $focus->list_type!='city') {
	$xtpl->assign("CITY_HIDDEN", "display:none");
}
if (empty($focus->list_type) or $focus->list_type!='experience') {
	$xtpl->assign("EXP_HIDDEN", "display:none");
}
if (empty($focus->list_type)) $focus->list_type=''; 
