...
// $Id: Popup_picker.php,v 1.7 2006/06/06 17:58:33 majed Exp $
...
class Popup_Picker
{
	...
	function _get_where_clause()
	{
		...
		{
			$where_clauses = array();
			append_where_clause($where_clauses, "first_name", "prospects.first_name");
	...
	function process_page()
	{
		...
		$form = new XTemplate('modules/Prospects/Popup_picker.html');
		...
		$ListView->setQuery($where, '', 'prospects.last_name,prospects.first_name', 'PROSPECT_POPUP');
		$ListView->setModStrings($mod_strings);

		ob_start();
		$output_html .= get_form_header($mod_strings['LBL_LIST_FORM_TITLE'], $button, false); //FOR MULTI-SELECT		
		$ListView->processListView($seed_bean, 'main', 'PROSPECT_POPUP');
		$output_html .= ob_get_contents();
		ob_end_clean();
