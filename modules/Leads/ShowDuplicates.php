...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_MODULE_NAME'].": ".$mod_strings['LBL_SAVE_LEAD'], true);
echo "\n</p>\n";
$xtpl=new XTemplate ('modules/Leads/ShowDuplicates.html');
...
$contactForm = new LeadFormBase();
$GLOBALS['check_notify'] = FALSE;

$query = 'select id,first_name, last_name, title, email1, email2  from leads where deleted=0 ';
...
$xtpl->assign('FORMBODY', $contactForm->buildTableForm($duplicateContacts,  'Leads'));

$input = '';
foreach ($contact->column_fields as $field)
{	
	if (!empty($_GET['Leads'.$field])) {
		$input .= "<input type='hidden' name='$field' value='${_GET['Leads'.$field]}'>\n";
	}
}
