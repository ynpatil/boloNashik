...
require_once ('modules/CountryMaster/Country.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/CountryMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/CountryMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/CountryMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'CountryMaster');
...
$seedCountry = new Country(); // seed bean
$searchForm = new SearchForm('CountryMaster', $seedCountry); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['CountryMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['CountryMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['CountryMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "CountryMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedCountry, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('CountryMaster');
