...
require_once ('modules/LinkageMaster/Linkage.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/LinkageMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/LinkageMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/LinkageMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'LinkageMaster');
...
$seedLinkage = new Linkage(); // seed bean
$searchForm = new SearchForm('LinkageMaster', $seedLinkage); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['LinkageMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['LinkageMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['LinkageMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "LinkageMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedLinkage, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('LinkageMaster');
