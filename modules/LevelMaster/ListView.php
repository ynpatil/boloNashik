...
require_once ('modules/LevelMaster/Level.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/LevelMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/LevelMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/LevelMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'LevelMaster');
...
$seedLevel = new Level(); // seed bean
$searchForm = new SearchForm('LevelMaster', $seedLevel); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['LevelMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['LevelMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['LevelMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "LevelMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedLevel, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('LevelMaster');
