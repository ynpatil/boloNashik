 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.72 2006/08/10 21:26:40 jenny Exp $
...
if(file_exists('custom/modules/Meetings/metadata/listviewdefs.php')){
	require_once('custom/modules/Meetings/metadata/listviewdefs.php');	
}else{
	require_once('modules/Meetings/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Meetings');
global $urlPrefix;
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')  
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Meetings');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Meetings'); // get user defined display columns

$json = getJSONobj();

$seedMeeting = new Meeting(); // seed bean
$searchForm = new SearchForm('Meetings', $seedMeeting); // new searchform instance
$show_who_has_access = "false";
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Meetings'][$col])) 
            $displayColumns[$col] = $listViewDefs['Meetings'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['Meetings'] as $col => $params) {
...
$other_user_list = getOtherUserIfAny(NULL,$seedMeeting->module_dir);
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Meetings"); // builds the where clause from search field inputs
...
$lv->setup($seedMeeting, 'include/ListView/ListViewGeneric.tpl', $where, $params);
