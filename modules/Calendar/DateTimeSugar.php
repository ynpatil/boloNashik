 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: DateTime.php,v 1.21 2006/08/12 01:29:14 jenny Exp $
 ********************************************************************************/
...
class DateTimeSugar
{
	...
	{	
		...
		ereg("([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})",$date_start,$match);
		$time_arr = array();
		...
		return new DateTimeSugar($time_arr,true);
	...
	function get_tomorrow()
	{
		...
		return new DateTimeSugar($date_arr,true);
	}
  ...
  function parse_utc_date_time($str)
  {
    ...
    $date_arr = array(
      ...
      $date_time = new DateTimeSugar($date_arr,true);
	...
	function DateTimeSugar(&$time,$fill_in_details)
	{	
	...
	function get_datetime_by_index_today($hour_index)
	{
		...
		return new DateTimeSugar($arr,true);
	}
	...
	function getHashList($view, &$start_time, &$end_time)
	{
		...
		if ( $new_time->ts == $end_time->ts)
		{
		 ...
		 while( $new_time->ts < $end_time->ts)
		 {
		  ...
		  $new_time = new DateTimeSugar($arr,true);
    }
		return $hash_list;
	}
