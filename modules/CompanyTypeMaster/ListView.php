...
require_once ('modules/CompanyTypeMaster/CompanyType.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/CompanyTypeMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/CompanyTypeMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/CompanyTypeMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'CompanyTypeMaster');
...
$seedCompanyType = new CompanyType(); // seed bean
$searchForm = new SearchForm('CompanyTypeMaster', $seedCompanyType); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['CompanyTypeMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['CompanyTypeMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['CompanyTypeMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "CompanyTypeMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedCompanyType, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('CompanyTypeMaster');
