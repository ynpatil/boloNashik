...
$log = LoggerManager::getLogger('usertype');

$seed_object = new UserType();
...
if(isset($_REQUEST['query']))
{
	...
	append_where_clause($where_clauses, "id", "usertype_mast.id");
	append_where_clause($where_clauses, "name", "usertype_mast.name");
...
if (!isset($_REQUEST['html'])) {
	$form =new XTemplate ('modules/UserTypeMaster/Popup_picker.html');
//	$log->debug("using file modules/UserTypeMaster/Popup_picker.html");
}
else {
//	$log->debug("_REQUEST['html'] is ".$_REQUEST['html']);
	$form =new XTemplate ('modules/UserTypeMaster/'.$_REQUEST['html'].'.html');
//	$log->debug("using file modules/UserTypeMaster/".$_REQUEST['html'].'.html');
}

$form->assign("MOD", $mod_strings);
$form->assign("APP", $app_strings);
if(isset($_REQUEST['usertype_source']))$usertype_source = $_REQUEST['usertype_source'];
...
elseif ($_REQUEST['form'] == 'UserTypeEditView')
{
	$the_javascript  = "<script type='text/javascript' language='JavaScript'>\n";
	$the_javascript .= "function set_return(parent_id, parent_name) {\n";
	//$the_javascript .= "    alert(parent_id+' '+parent_name); \n";
	$the_javascript .= "	window.opener.document.EditView.usertype_name.value = parent_name;\n";
	//$the_javascript .= "    alert('STEP 1'); \n";
	$the_javascript .= "	window.opener.document.EditView.usertype_id.value = parent_id;\n";
	$the_javascript .= "}\n";
	$the_javascript .= "</script>\n";
...
$ListView->setQuery($where, "", "description", "USERTYPE");
$ListView->setModStrings($mod_strings);
$ListView->processListView($seed_object, "main", "USERTYPE");
