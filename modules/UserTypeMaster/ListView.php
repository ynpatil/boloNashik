...
if(file_exists('custom/modules/UserTypeMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/UserTypeMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/UserTypeMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'UserTypeMaster');
...
$seedUserType = new UserType(); // seed bean
$searchForm = new SearchForm('UserTypeMaster', $seedUserType); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['UserTypeMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['UserTypeMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['UserTypeMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "UserTypeMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedUserType, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('UserTypeMaster');
