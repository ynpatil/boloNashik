...
require_once ('modules/CityMaster/City.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/CityMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/CityMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/CityMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'CityMaster');
...
$seedCity = new City(); // seed bean
$searchForm = new SearchForm('CityMaster', $seedCity); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['CityMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['CityMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['CityMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "CityMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedCity, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('CityMaster');
