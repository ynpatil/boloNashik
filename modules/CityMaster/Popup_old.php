...
$log = LoggerManager::getLogger('city');

$seed_object = new City();
...
if(isset($_REQUEST['query']))
{
	...
	append_where_clause($where_clauses, "id", "city_mast.id");
	append_where_clause($where_clauses, "name", "city_mast.name");
...
if (!isset($_REQUEST['html'])) {
	$form =new XTemplate ('modules/CountryMaster/Popup_picker.html');
	$log->debug("using file modules/CountryMaster/Popup_picker.html");
}
else {
	$log->debug("_REQUEST['html'] is ".$_REQUEST['html']);
	$form =new XTemplate ('modules/CountryMaster/'.$_REQUEST['html'].'.html');
	$log->debug("using file modules/CountryMaster/".$_REQUEST['html'].'.html');
}

$form->assign("MOD", $mod_strings);
$form->assign("APP", $app_strings);
if(isset($_REQUEST['country_source']))$country_source = $_REQUEST['country_source'];
...
elseif ($_REQUEST['form'] == 'CountryEditView')
{
	$the_javascript  = "<script type='text/javascript' language='JavaScript'>\n";
	$the_javascript .= "function set_return(parent_id, parent_name) {\n";
	//$the_javascript .= "    alert(parent_id+' '+parent_name); \n";
	$the_javascript .= "	window.opener.document.EditView.country_description.value = parent_name;\n";
	//$the_javascript .= "    alert('STEP 1'); \n";
	$the_javascript .= "	window.opener.document.EditView.country_id.value = parent_id;\n";
	$the_javascript .= "}\n";
	$the_javascript .= "</script>\n";
...
$ListView->setQuery($where, "", "description", "COUNTRY");
$ListView->setModStrings($mod_strings);
$ListView->processListView($seed_object, "main", "COUNTRY");
