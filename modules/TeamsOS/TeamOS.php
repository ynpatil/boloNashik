...
class TeamOS extends SugarBean {
    ...
    var $list_fields = array('name', 'deleted', 'date_entered', 'date_modified', 'email');
    var $required_fields = array('name','email');
    ...
    function fill_in_additional_detail_fields() {
        //Accessing level master name
//        $query = "SELECT name FROM level_mast WHERE id='$this->level_id'";        
//        $result = $this->db->query($query, true, "Error Getting level_mast table: ");              
//        $row = $this->db->fetchByAssoc($result);
//        $this->level_name = $row['name'];
//        
//        //
//        $query = "SELECT name FROM language_mast WHERE id='$this->language_id'";        
//        $result = $this->db->query($query, true, "Error Getting level_mast table: ");              
//        $row = $this->db->fetchByAssoc($result);
//        $this->language_name = $row['name'];
        
        //
//        $query = "SELECT exp_min,exp_max FROM experience_mast WHERE id='$this->experience_id'";        
//        $result = $this->db->query($query, true, "Error Getting level_mast table: ");              
//        $row = $this->db->fetchByAssoc($result);
//        $this->experience_name = $row['exp_min']? $row['exp_min']." TO ".$row['exp_max'] : "";
    }
    ...
    function setQuery($where, $module) {
        ...
        if (isset($moduleConfig[strtoupper($module)])) {
            ...
            $team_user_ids = array_keys(getUserMyTeam());
            
            if (count($teamArr) == 0) {
               /* Commented By Yogesh : if user don't have team then system will not show any data
                *  $new_where = "(" . $moduleConfig[$module] . "_cstm.assigned_team_id_c IS NULL OR ";
                $new_where .= $moduleConfig[$module] . "_cstm.assigned_team_id_c = \"None\" OR ";
                $new_where .= $moduleConfig[$module] . "_cstm.assigned_team_id_c = \"\" ";
                if ($module != "DOCUMENT" && $module != "NOTE") {
                    $new_where .= "OR " . $moduleConfig[$module] . ".assigned_user_id IN ('" . implode("','", $team_user_ids) . "')";
                    $new_where .= "OR " . $moduleConfig[$module] . ".created_by IN ('" . implode("','", $team_user_ids) . "'))";
...
//					$new_where .= "OR ". $moduleConfig[$module] .".assigned_user_id = '" . $current_user->id . "')";
    ...
    function get_all_members_for_team($id) {
        global $mod_strings;
        require_once("modules/Users/User.php");
        $focus_users = new User();
        $query = "SELECT * FROM team_membership WHERE team_id='$id'";
...
//						echo "Team id ".$team_id;
    ...
    function retrieve_team_list($user_id = '') {

        $team_array = $this->retrieve_team_id($user_id);
        $team_desc_array = array();

        foreach ($team_array as $value) {
            $team_desc_array[] = $this->get_assigned_team_name($value);
        }

        $GLOBALS['log']->debug("In retrieve_team_list :" . implode(",", $team_desc_array));
        return $team_desc_array;
    }
    ...
    function retrieve_team_id($user_id = '') {
        ...
        if (!empty($return_string) && $return_string != "") {
            ...
            $query_team = "SELECT teams.id as team_id
			                FROM team_membership as tm inner join teams 
			                ON (tm.team_id=teams.id AND
			                      tm.user_id IN (" . $return_string . ") AND
			                      teams.deleted=0 AND tm.deleted=0 AND teams.private=0)";
        ...
        $query_team = "SELECT teams.id as team_id
			                ...
			                ON (tm.team_id=teams.id AND
			                      ...
			                      teams.deleted=0 AND tm.deleted=0)";
    ...
    function can_access_record($action, $module, $is_owner) {
        return true;
//			return ACLController::checkAccess($module, $action, $is_owner);			
    }
    function getVendorsPercentage($campaign_id){                
                $query = "SELECT percentage FROM campaign_vendor 
                          WHERE vendor_id='$this->id' AND campaign_id = '$campaign_id' AND deleted='0' ";
