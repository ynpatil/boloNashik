...
// $Id: Popup_picker.php,v 1.24 2006/06/06 17:57:48 majed Exp $
	...
	function _get_where_clause()
	{
		...
		if(isset($_REQUEST['query']))
		{
			...
			$where = generate_where_statement($whereClauses);
                ...
                /* Added Search Functionality For Search by City , Zone, Product 
                 * because of City , Zone & Product are Added as SubPanel in Vendor 
                 * Module for that need extra search Functionality
                 */
                if($where){
                    $search_arr[] = " ";
                } 
                if($_REQUEST['city_id']){
                    $team_id_arr = getVendorIdByCityId($_REQUEST['city_id']);                    
                    $search_arr[]  = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";
                }
                if($_REQUEST['region_id']){                    
                    $team_id_arr = getVendorIdByZoneId($_REQUEST['region_id']);
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                    
                }                
                if($_REQUEST['product_id']){                    
                    $team_id_arr = getVendorIdByProductId($_REQUEST['product_id']);                    
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                
                }
                if($_REQUEST['state_id']){                    
                    $team_id_arr = getVendorIdByStateId($_REQUEST['state_id']);                    
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                
                }
                if($_REQUEST['language_id']){                    
                    $team_id_arr = getVendorIdByLanguageId($_REQUEST['language_id']);                    
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                
                }
                if($_REQUEST['level_id']){                    
                    $team_id_arr = getVendorIdByLevelId($_REQUEST['level_id']);                    
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                
                }
                if($_REQUEST['experience_id']){                    
                    $team_id_arr = getVendorIdByExperienceId($_REQUEST['experience_id']);                    
                    $search_arr[] = " teams.id IN  ('".implode("', '", $team_id_arr)."') ";                
                }
                if(is_array($search_arr))$where .= implode(" AND ", $search_arr);
                
		$GLOBALS['log']->info("Vendore ::Popup Picker WHERE : $where");
                $GLOBALS['log']->info("Vendore ::Popup Picker REQUEST :".  print_r($_REQUEST,true));
		return $where;
	}
...
//		echo "Where clause 1".$where."<br/>";
		...
		$form->assign('request_data', $request_data);
                ...
                $options_array = get_bean_select_array(true, 'Level', 'name', 'deleted=0', 'name',false);
		$form->assign("LEVEL_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['level_id']));
		$options_array = get_bean_select_array(true, 'Experience', 'name', 'deleted=0', 'name',false);
		$form->assign("EXPERIENCE_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['experience_id']));
		$options_array = get_bean_select_array(true, 'City', 'name', 'deleted=0', 'name',false);
		$form->assign("CITY_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['city_id']));
                
                $options_array = get_bean_select_array(true, 'State', 'name', 'deleted=0', 'name',false);
                $form->assign("STATE_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['state_id']));
                
                $options_array = get_bean_select_array(true, 'Language', 'name', 'deleted=0', 'name',false);
                $form->assign("LANGUAGE_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['language_id']));
                
                $options_array = get_bean_select_array(true, 'Region', 'name', 'deleted=0', 'name',false);
		$form->assign("REGION_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['region_id']));                
                $options_array = get_bean_select_array(true, 'Brand', 'name', 'deleted=0', 'name',false);
		$form->assign("PRODUCT_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['product_id']));
