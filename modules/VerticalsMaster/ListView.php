...
require_once ('modules/VerticalsMaster/Verticals.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/VerticalsMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/VerticalsMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/VerticalsMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'VerticalsMaster');
...
$seedVerticals = new Verticals(); // seed bean
$searchForm = new SearchForm('VerticalsMaster', $seedVerticals); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['VerticalsMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['VerticalsMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['VerticalsMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "VerticalsMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedVerticals, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('VerticalsMaster');
