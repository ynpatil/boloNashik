...
if(!$_REQUEST['new']) {
    if(!isset($_REQUEST['activity_details_for']) || $_REQUEST['activity_details_for'] == "Meetings") {
        require_once("modules/ActivityReport/MeetingListViewQueryMovementSummary.php");
        require_once("modules/ActivityReport/MeetingListViewDisplayMovementSummary.php");
    }

    if(!isset($_REQUEST['activity_details_for']) || $_REQUEST['activity_details_for'] == "Calls") {
        require_once("modules/ActivityReport/CallListViewQueryMovementSummary.php");
        require_once("modules/ActivityReport/CallListViewDisplayMovementSummary.php");
    }

    if(!isset($_REQUEST['activity_details_for']) || $_REQUEST['activity_details_for'] == "Task") {
        require_once("modules/ActivityReport/TaskListViewQueryMovementSummary.php");
        require_once("modules/ActivityReport/TaskListViewDisplayMovementSummary.php");
    }
    ...
    if(!isset($_REQUEST['activity_details_for']) || $_REQUEST['activity_details_for'] == "Users") {
        ...
        require_once("modules/ActivityReport/UserListViewDisplayMovementSummary.php");
    }
}
...
if(!isset($_REQUEST['activity_details_for'])) {
    ...
    $search_form=new XTemplate ('modules/ActivityReport/SearchFormMovementSummary.html');
...
if(count($branch_summary_list)>0 && !(isset($user) && !empty($user))) {
    ...
    $xtpl=new XTemplate ('modules/ActivityReport/OpenListViewMovementSummaryBranch.html');
...
if(count($suboffice_summary_list)>0 && !(isset($user) && !empty($user))) {
    ...
    $xtpl=new XTemplate ('modules/ActivityReport/OpenListViewMovementSummarySubOffice.html');
    $xtpl->assign("MOD", $current_module_strings);
    $xtpl->assign("APP", $app_strings);
    $xtpl->assign("THEME",$theme);
    $xtpl->assign("IMAGE_PATH", $image_path);
...
if(count($user_summary_list)>0) {
    ...
    $xtpl=new XTemplate ('modules/ActivityReport/OpenListViewMovementSummary.html');
    $xtpl->assign("MOD", $current_module_strings);
    $xtpl->assign("APP", $app_strings);
    $xtpl->assign("THEME",$theme);
    $xtpl->assign("IMAGE_PATH", $image_path);
    ...
    global $benchmark_red;
    global $benchmark_yellow;
    global $benchmark_green;
    global $odd_bg;
    global $even_bg;
    global $hilite_bg;
    global $click_bg;
    ...
    foreach($user_summary_list as $activity) {
        ...
        foreach($activity['COMPLIANCE'] as $compliance=>$value) {
//            $GLOBALS['log']->debug("Compliance :".$compliance." => ".$value);
            $compliance_data .= "<tr><td>".$compliance."</td><td onmouseover='' bgcolor=\"";
            if($value == "N.A")
                $compliance_data .= "\"";
            else{
                $value = intval($value);
                if($value < 65)
                    $compliance_data .= $benchmark_red."\"";
                else if($value>= 65 && $value < 85)
                        $compliance_data .= $benchmark_yellow."\"";
                    else if($value >= 85)
                            $compliance_data .= $benchmark_green."\"";
            }
            $compliance_data .= ">".$value."</td></tr>";
        }

        $compliance_data .= "</table>";
//        $GLOBALS['log']->debug("Compliance Data :".$compliance_data);
        $activity['COMPLIANCE'] = $compliance_data;
        $xtpl->assign("ACTIVITY", $activity);
...
/*
if(count($user_summary_list)>0) {
    echo "\n<p>Average Field Time-wise summary</p>\n";
    echo "\n<p>\n";

    $xtpl=new XTemplate ('modules/ActivityReport/OpenListViewMovementSummarySynopsis.html');
    $xtpl->assign("MOD", $current_module_strings);
    $xtpl->assign("APP", $app_strings);
    $xtpl->assign("THEME",$theme);
    $xtpl->assign("IMAGE_PATH", $image_path);
    ...
    if (count($user_summary_list) > 0) $user_summary_list = array_csort($user_summary_list, 'AVERAGE_FIELD_TIME', SORT_DESC);

    $ranges = Array(Array('from'=>1000,'to'=>100000000000000000000000000000000),Array('from'=>360,'to'=>999),Array('from'=>240,'to'=>359),Array('from'=>120,'to'=>239),Array('from'=>0,'to'=>119));
    $index = 0;
    $total_assigned = 0;

    foreach($ranges as $range) {
        $xtpl->reset("open_activity");
        $xtpl->assign("AVERAGE_FIELD_TIME", $range['from']." to ".$range['to']);
        $skip = true;

        $index = 0;
        foreach($user_summary_list as $activity) {
            if(!($activity['average_field_time'] >= $range['from'] && $activity['average_field_time'] <= $range['to']))continue;
           
            $skip = false;
//            if($index <5) {
                $activity1 = Array('USER'=>$activity['USER'],'REPORTS_TO_NAME' => $activity['REPORTS_TO_NAME']);
                $total_assigned++;
                $xtpl->assign("ACTIVITY", $activity1);
                $xtpl->parse("open_activity.row.column");
                $index++;
                if($index <5)
                continue;
//            }
        ...
        $xtpl->parse("open_activity.row");//send out any left blocks

        $xtpl->parse("open_activity");
//        if ($skip) $xtpl->reset("open_activity");
//        else
            if(!$skip)
...
echo "Total assigned user count :".$total_assigned;
*/
if($_REQUEST['print_pdf'] == 'true') {
    ...
    $pdf->y0=$pdf->GetY();
    $pdf->SetFont('Arial','',10);
    $header = array('Branch','Vertical','User','Meetings','Calls','Tasks','Avg Field Time(m)','Compliance %','Total Count');
    ...
    foreach($user_summary_list as $activity) {
        ...
        $entries[] = Array(
            ...
            'AVERAGE_FIELD_TIME'=> $activity['AVERAGE_FIELD_TIME'],
            'COMPLIANCE_PERCENTAGEE'=> $activity['COMPLIANCE_PERCENTAGE'],
