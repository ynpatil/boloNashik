...
if(file_exists('custom/modules/SubofficeMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/SubofficeMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/SubofficeMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'SubofficeMaster');
...
$seedSuboffice = new Suboffice(); // seed bean
$searchForm = new SearchForm('SubofficeMaster', $seedSuboffice); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['SubofficeMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['SubofficeMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['SubofficeMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "SubofficeMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedSuboffice, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('SubofficeMaster');
