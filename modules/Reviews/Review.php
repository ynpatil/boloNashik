 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: Review.php,v 1.134 2006/07/20 22:52:03 wayne Exp $
...
// Review is used to store customer information.
class Review extends SugarBean {
    var $field_name_map;
	...
	var $default_review_name_values = array('Please review ...');

	var $table_name = "reviews";

	var $object_name = "Review";
	var $module_dir = 'Reviews';
	var $review_for_module;
	...
	function create_list_query($order_by, $where, $show_deleted = 0)
	{
		$custom_join = $this->custom_fields->getJOIN();
            ...
            $query .= " FROM reviews ";
                            ...
                            ON reviews.assigned_user_id=users.id ";
	   if($custom_join){
   				$query .= $custom_join['join'];
 			}
 			 $where_auto = '1=1';
				if($show_deleted == 0){
                	$where_auto = " reviews.deleted=0 ";
...
//
//		$rel_name = "calls_reviews";
//		$result = $this->fill_review_for_details($rel_name);
//		if($result)return;
//		
//		$rel_name = "meetings_reviews";
//		$result = $this->fill_review_for_details($rel_name);
//		if($result)return;
		
	}
	
	function fill_review_for_details($rel_name){
		if(isset($this->$rel_name))
		{
			...
			foreach($result as $key){
				$this->parent_id = $key;
				$this->review_for_module = $this->$rel_name->getRelatedModuleName();
				$beanName = $beanList[$this->review_for_module];
...
//		echo "Parent name ".$this->parent_name;		
		...
		if ($this->status != "Completed" && $this->status != "Deferred" ) {
			$task_fields['SET_COMPLETE'] = "<a href='index.php?return_module=$currentModule&return_action=$action&return_id=" . ((!empty($focus->id)) ? $focus->id : "") . "&action=EditView&module=Reviews&record={$this->id}&status=Completed'>".get_image($image_path."close_inline","alt='Close' border='0'")."</a>";
		}
	...
	function set_notification_body($xtpl, $task)
	{
		...
		$xtpl->assign("REVIEW_SUBJECT", $task->name);
		$xtpl->assign("REVIEW_STATUS", (isset($task->status)?$app_list_strings['review_status_dom'][$task->status]:""));
		$xtpl->assign("REVIEW_DESCRIPTION", $task->description);
