 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.79 2006/08/10 21:26:40 jenny Exp $
...
if(file_exists('custom/modules/Reviews/metadata/listviewdefs.php')){
	require_once('custom/modules/Reviews/metadata/listviewdefs.php');
}else{
	require_once('modules/Reviews/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Reviews');
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Reviews');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Reviews'); // get user defined display columns

$json = getJSONobj();

$seedReview = new Review();
$searchForm = new SearchForm('Reviews', $seedReview); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Reviews'][$col]))
            $displayColumns[$col] = $listViewDefs['Reviews'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['Reviews'] as $col => $params) {
...
if(!isset($_REQUEST['query'])){
    $storeQuery->loadQuery('Reviews');
    $storeQuery->populateRequest();
}else{
    $storeQuery->saveFromGet('Reviews');
}
...
if(isset($_REQUEST['query']))
{
    // we have a query
    // first save columns
    $current_user->setPreference('ListViewDisplayColumns', $displayColumns, 0, 'Reviews');
    $searchForm->populateFromRequest(); // gathers search field inputs from $_REQUEST
    $where_clauses = $searchForm->generateSearchWhere(true, "Reviews"); // builds the where clause from search field inputs
...
$lv->setup($seedReview, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Reviews')));
$str = "<script>
