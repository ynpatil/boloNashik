...
require_once('modules/Messages/TreeData.php');
...
class Popup_Picker
{
	...
	function _get_where_clause()
	{
		...
		{
			$where_clauses = array();
			append_where_clause($where_clauses, "message_name", "messages.message_name");
			append_where_clause($where_clauses, "category_id", "messages.category_id");
			append_where_clause($where_clauses, "subcategory_id", "messages.subcategory_id");
			append_where_clause($where_clauses, "template_type", "messages.template_type");
			append_where_clause($where_clauses, "is_template", "messages.is_template");
	...
	function process_page()
	{
        ...
        global $sugar_config;
        $image_path = 'themes/'.$theme.'/images/';
        $b_from_messages=false;
        if (isset($_REQUEST['caller']) && $_REQUEST['caller']=='Messages') {
            $b_from_messages=true;
        }

        //initalize template
        $form = new XTemplate('modules/Messages/Popup_picker.html');
        ...
        $doctree->set_param('module','Messages');
        if ($b_from_messages) {
            $doctree->set_param('caller','Messages');
		...
		$message_name = empty($_REQUEST['message_name']) ? '' : $_REQUEST['message_name'];
        ...
        $form->assign('MESSAGE_NAME', $message_name);
		$form->assign('request_data', $request_data);
		$form->assign("CATEGORY_OPTIONS", get_select_options_with_id($app_list_strings['message_category_dom'], $category_id));
		$form->assign("SUB_CATEGORY_OPTIONS", get_select_options_with_id($app_list_strings['message_subcategory_dom'], $subcategory_id));
		$form->assign("IS_TEMPLATE_OPTIONS", get_select_options_with_id($app_list_strings['checkbox_dom'], $is_template));
		$form->assign("TEMPLATE_TYPE_OPTIONS", get_select_options_with_id($app_list_strings['message_template_type_dom'], $template_type));
		...
		$ListView->setQuery($where, '', 'message_name', 'MESSAGE');
		$ListView->setModStrings($mod_strings);

		ob_start();
		$ListView->processListView($seed_bean, 'main', 'MESSAGE');
		$output_html .= ob_get_contents();
		ob_end_clean();
