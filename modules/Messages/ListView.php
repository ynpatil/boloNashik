...
if(file_exists('custom/modules/Messages/metadata/listviewdefs.php')){
	require_once('custom/modules/Messages/metadata/listviewdefs.php');
}else{
	require_once('modules/Messages/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Messages');
...
$seedMessage = new Message(); // seed bean
$searchForm = new SearchForm('Messages', $seedMessage); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Messages'][$col]))
            $displayColumns[$col] = $listViewDefs['Messages'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['Messages'] as $col => $params) {
...
//echo "Columns :".implode(",",array_keys($lv->displayColumns));
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Messages"); // builds the where clause from search field inputs
...
$lv->setup($seedMessage, 'include/ListView/ListViewGeneric.tpl', $where, $params);
