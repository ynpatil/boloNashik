...
function get_node_data($params,$get_array=false) {
    ...
    $href=true;
    if (isset($params['TREE']['caller']) and $params['TREE']['caller']=='Messages' ) {
        $href=false;
    }
	$nodes=get_messages($cat_id,$subcat_id,$href);
 ...
 */
 function get_category_nodes($href_string){
    ...
    $query="select distinct category_id from messages where deleted=0 order by category_id, subcategory_id";
    ...
    while (($row=$GLOBALS['db']->fetchByAssoc($result))!= null) {
        ...
        if (empty($row['category_id'])) {
            ...
            $cat_name=$app_list_strings['message_category_dom'][$row['category_id']];
        }
...
function get_messages($cat_id,$href=true) {
    $nodes=array();
    $href_string = "javascript:select_message('doctree')";
    ...
    $current_cat_id=null;
    while (($row=$GLOBALS['db']->fetchByAssoc($result))!= null) {
        $node = new Node($row['id'], $row['message_name']);
        if ($href) {
