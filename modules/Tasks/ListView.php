...
if(file_exists('custom/modules/Tasks/metadata/listviewdefs.php')){
	require_once('custom/modules/Tasks/metadata/listviewdefs.php');	
}else{
	require_once('modules/Tasks/metadata/listviewdefs.php');
}
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')  
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Tasks');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Tasks'); // get user defined display columns

$json = getJSONobj();

$seedTask = new Task();
$searchForm = new SearchForm('Tasks', $seedTask); // new searchform instance
...
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Tasks'] as $col => $params) {
...
$other_user_list = getOtherUserIfAny(NULL,$seedTask->module_dir);
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Tasks"); // builds the where clause from search field inputs
