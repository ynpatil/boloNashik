...
if(ACLController::checkAccess('Calls', 'list', true))$module_menu[]=Array("index.php?module=Calls&action=index&return_module=Calls&return_action=DetailView", $app_strings['LNK_CALL_LIST_MOBILE'],"Calls");
if(ACLController::checkAccess('Meetings', 'list', true))$module_menu[]=Array("index.php?module=Meetings&action=index&return_module=Meetings&return_action=DetailView", $app_strings['LNK_MEETING_LIST_MOBILE'],"Meetings");
if(ACLController::checkAccess('Tasks', 'list', true))$module_menu[]=Array("index.php?module=Tasks&action=index&return_module=Tasks&return_action=DetailView", $app_strings['LNK_TASK_LIST_MOBILE'],"Tasks");
...
if(ACLController::checkAccess('Comments', 'list', true))$module_menu[]=Array("index.php?module=Comments&action=index&return_module=Comments&return_action=DetailView", $app_strings['LNK_COMMENT_LIST'],"Comments");
...
if(ACLController::checkAccess('Tasks', 'list', true))$module_menu[]=Array("index.php?module=Tasks&action=RequestsListView&return_module=Tasks&return_action=DetailView", $mod_strings['LNK_TASK_REQUESTS'],"Tasks", 'Tasks');
...
if(ACLController::checkAccess('Activities', 'list', true))$module_menu[]=Array("index.php?module=ActivityReport&action=index&step=1&return_module=Activities&return_action=index", 'Activity Report',"ActivityReport");

global $current_user;
if(is_admin($current_user) || is_supersenior($current_user)){
	$module_menu[] = Array("index.php?module=ActivityReport&action=summary",$app_strings['LNK_ACTIVITY_REPORT_SUMMARY'],"ActivityReport");
}
