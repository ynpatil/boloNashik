...
require_once('modules/AccountObjective/Dashlets/dashlets.php');

$columns = $current_user->getPreference('columns', 'accountobj');
$dashlets = $current_user->getPreference('dashlets', 'accountobj');

// fill in with default dashlet selection
if(!isset($columns) || !isset($dashlets)) {
    $dashlets = array();

    $defaultDashlets = array('MktObjDashlet','CommObjDashlet','MktPriDashlet','LatestHappenDashlet');
    ...
    $current_user->setPreference('dashlets', $dashlets, 0, 'accountobj');
    $current_user->setPreference('columns', $columns, 0, 'accountobj');
}
...
echo $sugar_smarty->fetch('modules/AccountObjective/DetailView.tpl');
