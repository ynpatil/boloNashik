...
function sodUpdateProgressBar($itemweight){
    ...
    {
      $progress_bar_percent += ($itemweight / $totalweight * 100);
...
function getFullTableDump($tableName){
	...
	else
	{
		...
		$def_count = 0;
		while($row = mysql_fetch_row($def_result))
		{
	...
	{
		$row_counter = 1;
		while($row = mysql_fetch_row($td_result))
		{
			$returnString .= "<tr>";
...
function executesugarlog()
{
    ...
    if(!copy('sugarcrm.log', $cacheDir.'/sugarcrm.log')) {
...
function executephpinfo()
{
    ...
    $handle = fopen($cacheDir."phpinfo.html", "w");
...
function executemysql($getinfo, $getdumps, $getschema)
{
    ...
    if($getinfo)
    {
      ...
      echo "MySQL Host Info: ".mysql_get_host_info()."<BR>";
      echo "MySQL Server Info: ".mysql_get_client_info()."<BR>";
      echo "MySQL Client Encoding: ".mysql_client_encoding()."<BR>";
      
      /* Uncomment to get current processes as well
      echo "<BR>MySQL Processes<BR>";
      $res = mysql_list_processes($db->getInstance());
      ...
      */
      ...
      $handle = fopen($mysqlInfoDir."MySQL-General-info.html", "w");
    ...
    if($getschema)
    {
        ...
        foreach($all_tables as $tablename){
			...
			while($inner_row = mysql_fetch_row($describe)){
		...
		$handle = fopen($tablesSchemaDir."MySQLTablesSchema.html", "w");
		if(fwrite($handle, $content) === FALSE){
    ...
    if($getdumps)
    {
		...
		foreach ($getDumpsFrom as $table)
		{
			...
			$content = ob_get_contents();
			ob_clean(); 
			$handle = fopen($tableDumpsDir.$table.".html", "w");
			if(fwrite($handle, $content) === FALSE){
...
function executebeanlistbeanfiles()
{
	...
	$handle = fopen($cacheDir."beanFiles.html", "w");
	if(fwrite($handle, $content) === FALSE){
...
function executevardefs()
{
    ...
    $handle = fopen($cacheDir."vardefschema.html", "w");
...
function finishDiag(){
	...
	print "<a href=\"cache/diagnostic/".$sod_guid."/diagnostic".$curdatetime.".zip\">".$mod_strings['LBL_DIAGNOSTIC_DOWNLOADLINK']."</a><BR>";
