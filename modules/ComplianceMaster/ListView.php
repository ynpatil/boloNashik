...
if(file_exists('custom/modules/ComplianceMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/ComplianceMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/ComplianceMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ComplianceMaster');
...
$seedCompliance = new Compliance(); // seed bean
$searchForm = new SearchForm('ComplianceMaster', $seedCompliance); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ComplianceMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['ComplianceMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['ComplianceMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ComplianceMaster"); // builds the where clause from search field inputs
...
echo "<br/><a class=\"listViewTdLinkS1\" href='index.php?module=$currentModule&return_action=ListView&return_module=$currentModule&action=EditView'>Create/Update record</a>";

$lv->setup($seedCompliance, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('ComplianceMaster');
