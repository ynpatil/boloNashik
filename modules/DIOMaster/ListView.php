...
if(file_exists('custom/modules/DIOMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/DIOMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/DIOMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'DIOMaster');
...
$seedDIO = new DIO(); // seed bean
$searchForm = new SearchForm('DIOMaster', $seedDIO); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['DIOMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['DIOMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['DIOMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "DIOMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedDIO, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('DIOMaster');
