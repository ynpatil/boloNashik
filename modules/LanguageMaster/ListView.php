...
require_once ('modules/LanguageMaster/Language.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/LanguageMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/LanguageMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/LanguageMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'LanguageMaster');
...
$seedLanguage = new Language(); // seed bean
$searchForm = new SearchForm('LanguageMaster', $seedLanguage); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['LanguageMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['LanguageMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['LanguageMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "LanguageMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedLanguage, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('LanguageMaster');
