...
function checkForDuplicates($prefix){
	...
	$focus = new Contact();
	$query = '';
	$baseQuery = 'select contacts.id,contacts.first_name, contacts.last_name, contacts.title, contacts.email1, contacts.email2  from contacts ';


	if(!empty($_POST[$prefix.'account_id'])){
		$baseQuery .= " INNER JOIN accounts_contacts ON contacts.id = accounts_contacts.contact_id AND accounts_contacts.account_id='".$_POST[$prefix.'account_id']."' AND accounts_contacts.deleted!=1 ";
	}

	$baseQuery .= ' where contacts.deleted!=1 and (';
	...
	if(!empty($_POST[$prefix.'phone_mobile'])){
		if(!empty($query))
		$query .= ' AND ';
		$query = $query ."  (phone_mobile like '".$_POST[$prefix.'phone_mobile']."%')";
	}
	if(!empty($_POST[$prefix.'phone_work'])){
		if(!empty($query))
		$query .= ' AND ';
		$query = $query ."  (phone_work like '".$_POST[$prefix.'phone_work']."%')";
	}
	if(!empty($_POST[$prefix.'phone_home'])){
		if(!empty($query))
		$query .= ' AND ';
		$query = $query ."  (phone_home like '".$_POST[$prefix.'phone_home']."%')";
	}
	if(!empty($query)){
		$rows = array();
		global $db;
		...
		if ($i==-1)
			return $this->checkCompressString($_POST[$prefix.'first_name'].$_POST[$prefix.'last_name'].$_POST[$prefix.'account_id']);
		else return $rows;
	}
	return null;
}
...
function computeHash($string){
	...
	$str = SugarAuthenticate::encodePassword($str);
	return $str;
}

function checkCompressString($string){

	global $db;

	$str = $this->computeHash($string);
	$GLOBALS['log']->debug("Compressed String :".$str);

	$ids = array();
	$query = "SELECT entity_id from check_duplicate_contacts where id='$str'";
	$result = $db->query($query);
	...
	if ($i==-1)
		return null;
		 ...
		 $query = "select id,first_name, last_name, title, email1, email2  from contacts where deleted!=1 ".
		 " and id in ('".implode("','",$ids)."')";

		$GLOBALS['log']->debug("SQL query in ContactFormBase.compressString :".$query);
		$result = $db->query($query);
		$i=-1;
		$rows = array();
...
function getWideFormBody($prefix, $mod='',$formname='',  $contact = '', $portal = true){
	...
	$form .= <<<EOQ
		...
		<tr>
		<td class='dataField'><input name='${prefix}primary_address_city_desc'  maxlength='100' value='{$contact->primary_address_city_desc}'></td>
		<td class='dataField'><input name='${prefix}primary_address_state_desc'  maxlength='100' value='{$contact->primary_address_state_desc}'></td>
		...
		<td class='dataField'><input name='${prefix}primary_address_country_desc'  maxlength='100' value='$contact->primary_address_country_desc'></td>
		</tr>


		<tr>
		<td nowrap class='dataLabel'>$lbl_phone</td>
		<td nowrap class='dataLabel'>${mod_strings['LBL_MOBILE_PHONE']}&nbsp;<span class="required">$lbl_required_symbol</span></td>
...
$form = substr($form,0,strlen($form)-strlen(strrchr($form,"<br>"))); //remove the trailing <br>
$form .= $lbl_teams . "<br><select name='assigned_team_id_c' id='assigned_team_id_c' style='width: 120px'>";
$form .= $focus_team->get_default_team_select($current_user->default_team_id_c, $current_user->id);
$form .= "</select><br><br>";
/* end Lampada change */
require_once('include/javascript/javascript.php');
require_once('modules/Contacts/Contact.php');
...
function deleteCompressStringForBean($focus){

	global $db;
	$query = "DELETE FROM check_duplicate_contacts where entity_id='$focus->id'";
	$GLOBALS['log']->debug("In deleteCompressString :".$query);
	$db->query($query,true);
}

function saveCompressStringForBean($focus){

	global $db;
	$hash = $this->computeHash($focus->first_name.$focus->last_name.$focus->account_id);
	$query = "INSERT INTO check_duplicate_contacts set id='$hash',entity_id='$focus->id'";
	$GLOBALS['log']->debug("In saveCompressString :".$query);
	$db->query($query,true);
}
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	$focus->save($check_notify);
	$this->saveCompressStringForBean($focus);
