...
if(file_exists('custom/modules/BranchMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/BranchMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/BranchMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'BranchMaster');
...
$seedBranch = new Branch(); // seed bean
$searchForm = new SearchForm('BranchMaster', $seedBranch); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['BranchMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['BranchMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['BranchMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "BranchMaster"); // builds the where clause from search field inputs
...
$savedSearchResult = $savedSearch->getSelect('BranchMaster');
