...
require_once('modules/Transfers/Forms.php');
$header_text = '';
global $mod_strings;
...
$GLOBALS['log']->info("Transfer list view");
global $theme;
...
$button .= "<input type='hidden' name='module' value='Transfers'>\n";
...
$ListView->initNewXTemplate( 'modules/Transfers/ListView.html',$mod_strings);
...
if ($is_edit) {
		...
		$edit_button .="<input type='hidden' name='return_module' value='Transfers'>\n";
		$edit_button .="<input type='hidden' name='return_action' value='index'>\n";
		$edit_button .="<input type='hidden' name='return_id' value=''>\n";
		$edit_button .='<input title="'.$mod_strings['LBL_START_BUTTON_TITLE'].'" accessKey="'.$mod_strings['LBL_TRANSFER_BUTTON_KEY'].'" class="button" onclick="this.form.action.value=\'StartTransfer\'; return check_form(\'EditView\');" type="submit" name="button" value="  '.$mod_strings['LBL_TRANSFER_BUTTON_LABEL'].'  " >';
//		$edit_button .=' <input title="'.$app_strings['LBL_SAVE_NEW_BUTTON_TITLE'].'" accessKey="'.$app_strings['LBL_SAVE_NEW_BUTTON_KEY'].'" class="button" onclick="this.form.action.value=\'Save\'; this.form.isDuplicate.value=\'true\'; this.form.edit.value=\'true\'; this.form.return_action.value=\'EditView\'; return check_form(\'EditView\')" type="submit" name="button" value="  '.$app_strings['LBL_SAVE_NEW_BUTTON_LABEL'].'  " >';
	...
	$GLOBALS['log']->info("Transfers edit view");
	$xtpl=new XTemplate ('modules/Transfers/EditView.html');
	...
	$xtpl->assign("USER_FILTER_FROM", get_select_options_with_id(get_user_array(TRUE, "Active"),$focus->user_from_id));
	$xtpl->assign("USER_FILTER_TO", get_select_options_with_id(get_user_array(TRUE, "Active"),$focus->user_to_id));
	$xtpl->assign("ACTIVITY",$focus->activity);
	if(isset($focus->created_by))
	$xtpl->assign("CREATED_BY","Transfer done by ".get_assigned_user_name($focus->created_by));
	if(isset($focus->date_entered))
	$xtpl->assign("CREATED_ON","Transfer done on ".$focus->date_entered);
		
	include_once("modules/Transfers/transferModules.php");
//	$GLOBALS['log']->debug("Activity list :".implode("/",$focus->activity));
	
	foreach($moduleTransferList as $module){
//		$GLOBALS['log']->debug("Module :".$module. " key exists :".array_key_exists($module,$focus->activity));
		$xtpl->assign("CHECKED","");
		
		if(is_array($focus->modules)){
			foreach($focus->modules as $moduleName){
				
				if($moduleName == $module){
//					$GLOBALS['log']->debug("Checking ".$moduleName." with :".$module);
		...
		$xtpl->assign("MODULE_NAME",$app_list_strings['moduleList'][$module]);
