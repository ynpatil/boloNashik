...
require_once ('modules/ExperienceMaster/Experience.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/ExperienceMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/ExperienceMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/ExperienceMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ExperienceMaster');
...
$seedExperience = new Experience(); // seed bean
$searchForm = new SearchForm('ExperienceMaster', $seedExperience); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ExperienceMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['ExperienceMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['ExperienceMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ExperienceMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedExperience, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('ExperienceMaster');
