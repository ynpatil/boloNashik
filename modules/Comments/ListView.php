...
if(file_exists('custom/modules/Comments/metadata/listviewdefs.php')){
	require_once('custom/modules/Comments/metadata/listviewdefs.php');
}else{
	require_once('modules/Comments/metadata/listviewdefs.php');
}
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Comments');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Comments'); // get user defined display columns

$json = getJSONobj();

$seedComment = new Comment();
$searchForm = new SearchForm('Comments', $seedComment); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Comments'][$col]))
            $displayColumns[$col] = $listViewDefs['Comments'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['Comments'] as $col => $params) {
...
if(!isset($_REQUEST['query'])){
    $storeQuery->loadQuery('Comments');
    $storeQuery->populateRequest();
}else{
    $storeQuery->saveFromGet('Comments');
}
...
if(isset($_REQUEST['query']))
{
    // we have a query
    // first save columns
    $current_user->setPreference('ListViewDisplayColumns', $displayColumns, 0, 'Comments');
    $searchForm->populateFromRequest(); // gathers search field inputs from $_REQUEST
    $where_clauses = $searchForm->generateSearchWhere(true, "Comments"); // builds the where clause from search field inputs
...
$lv->setup($seedComment, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Comments')));
$str = "<script>
