 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php,v 1.69 2006/08/10 21:26:40 jenny Exp $
...
if(file_exists('custom/modules/Calls/metadata/listviewdefs.php')){
	require_once('custom/modules/Calls/metadata/listviewdefs.php');	
}else{
	require_once('modules/Calls/metadata/listviewdefs.php');
}
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')  
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Calls');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Calls'); // get user defined display columns

$json = getJSONobj();

$seedCall = new Call();
$searchForm = new SearchForm('Calls', $seedCall); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Calls'][$col])) 
            $displayColumns[$col] = $listViewDefs['Calls'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Calls'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Calls"); // builds the where clause from search field inputs
...
$lv->setup($seedCall, 'include/ListView/ListViewGeneric.tpl', $where, $params);
