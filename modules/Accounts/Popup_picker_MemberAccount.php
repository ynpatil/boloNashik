...
class Popup_Picker_MemberAccount
{
	var $_popupMeta;
	var $_create = false;
	 ...
	 * 
	 */
	function Popup_Picker_MemberAccount()
	{
...
EOQ;
		...
		if (!empty($_REQUEST['mode']) && strtoupper($_REQUEST['mode']) == 'MULTISELECT') {
			...
			$button .= "<input type='button' name='button' class='button' onclick=\"send_back_selected1('$currentModule',document.MassUpdate,'mass[]','" .$app_strings['ERR_NOTHING_SELECTED']."', request_data.field_to_name_array);\" title='"
		...
		{
			$button .= "<input type='button' name='button' class='button' onclick=\"send_back1('','');\" title='"
		...
		if(isset($this->_popupMeta['templateForm'])) { 
			...
			$form = new XTemplate('modules/' . $currentModule . '/Popup_picker_MemberAccount.html');
		}
		...
		$form->assign("CITY_OPTIONS", get_select_options_with_id($options_array,$_REQUEST['billing_address_city']));
