...
$query = 'select accounts.id, accounts.name, accounts.website, city_mast.name as billing_address_city from accounts inner join city_mast on accounts.billing_address_city = city_mast.id where accounts.deleted=0 ';
...
if ($count > 0)
{
	...
	foreach ($duplicates as $duplicate_id) 
	{
		if (!$first) $query .= ' OR ';
		$first = false;
		$query .= "accounts.id='$duplicate_id' ";
	}
	$query .= ')';
}
