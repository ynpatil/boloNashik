...
function checkForDuplicates($prefix){
	...
	if(!empty($_POST[$prefix.'phone_office'])){
		if(!empty($query))
		$query .= ' AND ';
		$query = $query ."  (phone_office like '".$_POST[$prefix.'phone_office']."%')";
	}

	if(!empty($_POST[$prefix.'email1'])){
		if(!empty($query))
		$query .= ' OR ';
		$query = $query ."  (email1 like '".$_POST[$prefix.'email1']."%')";
	}
...
//	$GLOBALS['log']->debug("Accounts duplicate query :".$query);
	...
	if(!empty($query)){
		...
		if ($i==-1)
			return $this->checkCompressString($_POST[$prefix.'name'].$_POST[$prefix.'phone_office']);
		else return $rows;
	}
	return null;
}
...
//	$str = preg_replace('/\s\s+/', ' ', $str);
	$str = strtolower($str);
	$GLOBALS['log']->debug("Compressed String before hash:".$str." org str :".$string);
...
function deleteCompressStringForBean($focus){
	...
	global $db;
	$query = "DELETE FROM check_duplicate_accounts where entity_id='$focus->id'";
...
function checkCompressString($string){
	...
	$ids = array();
	$query = "SELECT entity_id from check_duplicate_accounts where id='$str'";
	$result = $db->query($query);
...
$form = substr($form,0,strlen($form)-strlen(strrchr($form,"</p>"))); //remove the trailing <br>
...
function saveCompressStringForBean($focus){
	...
	global $db;
	$hash = $this->computeHash($focus->name.$focus->phone_office);
	$query = "INSERT INTO check_duplicate_accounts set id='$hash',entity_id='$focus->id'";
	$GLOBALS['log']->debug("In saveCompressStringForBean :".$query);
	$db->query($query,true);
}
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if($_REQUEST['source_info'] == "AccountRequest"){

		require_once("modules/Accounts/AccountRequest.php");
		$focusRequest = new AccountRequest();
