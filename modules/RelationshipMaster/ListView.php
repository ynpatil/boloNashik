...
require_once ('modules/RelationshipMaster/RelationshipMaster.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/RelationshipMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/RelationshipMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/RelationshipMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'RelationshipMaster');
...
$seedRelationshipMaster = new RelationshipMaster(); // seed bean
$searchForm = new SearchForm('RelationshipMaster', $seedRelationshipMaster); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['RelationshipMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['RelationshipMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['RelationshipMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "RelationshipMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedRelationshipMaster, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('RelationshipMaster');
