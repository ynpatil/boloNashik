...
<?php
	...
	function template_echo_slice_activities_shared(& $args) {
		...
		foreach($args['slice']->acts_arr[$shared_user->id] as $act) {
			...
			/* begin Lampada change */
/*
			$employees = array_keys(get_user_array_forassign(FALSE));
			if(is_admin($current_user) || $shared_user->id == $current_user->id || in_array($shared_user->id, $employees)) {
			   	$act->sugar_bean->name=$current_user->name;
			}
*/			
			/* end Lampada change */
	...
	function template_calendar(& $args) {
		...
		if(isset($_REQUEST['view']) && $_REQUEST['view'] == 'shared') {
			...
			elseif(isset($_REQUEST['shared_ids']) && count($_REQUEST['shared_ids']) > 0) {
				...
				$ids = get_user_array_forassign(FALSE);//get_user_array(false);
				$ids = array_keys($ids);
			}
...
/*
			if(!empty($team_id)) {
				...
				$users = get_user_array_forassign(FALSE);//$team->get_team_members();
				$user_ids = array();
...
*/			
			echo get_select_options_with_id(get_user_array_forassign(FALSE), $ids);
