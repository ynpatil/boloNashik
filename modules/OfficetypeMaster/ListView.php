...
require_once ('modules/OfficetypeMaster/Officetype.php');
require_once ('themes/'.$theme.'/layout_utils.php');
require_once ('log4php/LoggerManager.php');
require_once('include/ListView/ListViewSmarty.php');
if(file_exists('custom/modules/OfficetypeMaster/metadata/listviewdefs.php')){
    require_once('custom/modules/OfficetypeMaster/metadata/listviewdefs.php');
}else{
    require_once('modules/OfficetypeMaster/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'OfficetypeMaster');
...
$seedOfficetype = new Officetype(); // seed bean
$searchForm = new SearchForm('OfficetypeMaster', $seedOfficetype); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['OfficetypeMaster'][$col]))
            $displayColumns[$col] = $listViewDefs['OfficetypeMaster'][$col];
    }
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['OfficetypeMaster'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "OfficetypeMaster"); // builds the where clause from search field inputs
...
$lv->setup($seedOfficetype, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchResult = $savedSearch->getSelect('OfficetypeMaster');
