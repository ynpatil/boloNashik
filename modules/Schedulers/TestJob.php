...
if ($focus->job) {
    $JobFunctionArray = explode("::", $focus->job);
    if (is_array($JobFunctionArray)) {
        // instantiate a new SchedulersJob object and prep it 
        require_once('modules/SchedulersJobs/SchedulersJob.php');
        $job = new SchedulersJob();
        $job->scheduler_id = $focus->id;
        $job->scheduler = & $focus;
        $job->execute_time = gmdate("d-m-Y H:ia");
        $jobId = $job->save();
        $job->retrieve($jobId);

        $job->setCustomJobFlag(1);
        $JobFunctionName = $JobFunctionArray[1];
        $JobFunctionResult = $JobFunctionName();

        if (is_array($JobFunctionResult)) {
            if ($JobFunctionResult[0]) {
                $job->setCustomJobFlag(2,  addslashes($JobFunctionResult[1]));
                $job->finishJob();
                
            } else {
                $job->setCustomJobFlag(3,  addslashes($JobFunctionResult[1]));
                
            }
            $status=$JobFunctionResult[0];
        }else{
            if($JobFunctionResult){
                $job->setCustomJobFlag(2);
                $job->finishJob();
            }else{
                $job->setCustomJobFlag(3);
            }
            $status=$JobFunctionResult;
...
echo "<BR><B>JOB:   $JobFunctionName is Executed.<BR><br> STATUS : " . $status . "</b>";
?>
