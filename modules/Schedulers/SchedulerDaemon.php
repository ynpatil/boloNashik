...
//		_ppd($validJobTime);
			...
			foreach($jobsArr['ids'] as $k => $ids) {
				foreach($jobsArr['times'][$k] as $j => $time) {
					...
					$q = "INSERT INTO schedulers_times
						...
						'".$guid."',
						0, 
						".db_convert("'".gmdate('Y-m-d H:i:s')."'", 'datetime').",
						".db_convert("'".gmdate('Y-m-d H:i:s')."'", 'datetime').",
						'".$jobsArr['ids'][$k]."',
	...
	function retrieveSchedulers() {
		...
		$query 	= "SELECT id " .
				...
				"WHERE deleted=0 " .
				"AND status = 'Active' " .
				"AND date_time_start < ".db_convert("'".gmdate('Y-m-d H:i:s')."'",'datetime')." " .
				"AND (date_time_end > ".db_convert("'".gmdate('Y-m-d H:i:s')."'",'datetime')." OR date_time_end IS NULL)";
