...
//		echo "type=$type id=$id  \n";
		$GLOBALS['log']->debug("Type :".$type." = ".$id);
		...
		if($first)
		{
			...
			{
				$sql  = "SELECT ";
				$sql .= "  '' as reports_to_id, accounts.id, accounts.name, industry_mast.name as tooltip, 'Accounts' as type, accounts.assigned_user_id, accounts.id as acc_id ";
				$sql .= "FROM ";
				$sql .= "  accounts LEFT JOIN industry_mast ON accounts.industry = industry_mast.id ";
				$sql .= "WHERE ";
				$sql .= "  accounts.deleted=0 AND accounts.id = '$id'";
				//echo $sql;
			}
			elseif($type == 'Contacts')
			{
				$sql  = "SELECT ";
				$sql .= "  reports_to_id, id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip,function_mast.name as function_name, 'Contacts' as type, contacts.assigned_user_id, '' as acc_id ";
				$sql .= "FROM ";
			...
			{
				$sql  = "SELECT ";
				$sql .= "  '' as reports_to_id, '' as id, '$company' as name, '' as tooltip, '$type' as type, '3' as assigned_user_id, '' as acc_id, $status_fld as status FROM users LIMIT 1";
			}
		}
		else
		{
			...
			{
				$sql = "(SELECT ";
				$sql .= "  contacts.reports_to_id as reports_to_id, contacts.id as id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip, function_mast.name as function_name,'Contacts' as type, contacts.assigned_user_id, account_id as acc_id ";
				$sql .= "FROM ";
				...
				$sql .= "UNION ALL ";
				$sql .= "(SELECT ";
				$sql .= "  '', accounts.id, accounts.name, industry_mast.name as tooltip,'' as function_name, 'Accounts' as type, accounts.assigned_user_id, accounts.id ";
				$sql .= "FROM ";
				$sql .= "  accounts LEFT JOIN industry_mast ON accounts.industry = industry_mast.id ";
				$sql .= "WHERE ";
				$sql .= "  accounts.deleted=0 AND parent_id = '$id') ";
				$sql .= "UNION ALL ";
				$sql .= "(SELECT ";
				$sql .= "  '', a3.id, a3.name as name, industry_mast.name as tooltip,'AOR' as function_name, 'Accounts' as type, a3.assigned_user_id, a3.id ";
				$sql .= "FROM ";
				$sql .= "  accounts a1 INNER JOIN accounts a2 ON a1.id = a2.id AND a1.id = '$id' ";
				$sql .= " INNER JOIN accounts a3 ON a1.aor_id = a3.id ";
				$sql .= " LEFT JOIN industry_mast ON a3.industry = industry_mast.id ";
				$sql .= " WHERE ";
				$sql .= "  a3.deleted=0 ) ";
				$sql .= "ORDER BY ";
...
//					$GLOBALS['log']->debug("Can Access Record ".$current_team);
					...
					if($row['type'] != 'Contacts' || !$security || $current_team->can_access_record($row['type'], 'view',$owner))
						$link .= "<a status='' title='" . strip_tags($row['tooltip']) . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name'] . "<hr>" . $row['description'] . "<hr>" . $row['function_name'];
					else
