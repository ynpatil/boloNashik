...
//		if(!empty($curr_id)) {
			get_children($parent_id, true, $parent_id, 'Brands');
//		}
		add_footer();
	}

	function get_top_company($parent_id, $type='Brands')
	{
		$companies = array();
		$users = array();
		$curr_id = $parent_id;
		...
		if($type == 'Brands')
		{
			$sql = "SELECT account_id as parent_id FROM brands WHERE deleted=0 AND id = '$curr_id'";
			$result = $db->query($sql);
	...
	function get_children($id, $first=false, $focus_id='', $type='Accounts')
	{
		...
		if($first)
		{
			...
			{
				$sql  = "SELECT ";
				$sql .= "  '' as reports_to_id, id, name, name as tooltip, 'Brands' as type, assigned_user_id, account_id as acc_id ";
				$sql .= "FROM ";
				$sql .= "  brands ";
				$sql .= "WHERE ";
				$sql .= "  deleted=0 AND id = '$focus_id'";
			}			
		}
		else
		{
			...
			{
				$sql  = "(SELECT ";
				$sql .= "  '' as reports_to_id, accounts.id, accounts.name, industry as tooltip, 'Accounts' as type, accounts.assigned_user_id, accounts.id as acc_id,'Primary Account' as status ";
				$sql .= " FROM ";
				$sql .= "  accounts INNER JOIN brands ON brands.account_id = accounts.id";
				$sql .= " WHERE ";
				$sql .= " brands.id = '$id' AND accounts.deleted=0 and brands.deleted=0)";
				$sql .= " UNION ALL ";
				$sql .= "(SELECT ";
				$sql .= "  '' as reports_to_id, accounts.id, accounts.name, industry as tooltip,'Accounts' as type, accounts.assigned_user_id, ' ' as acc_id,' ' as status  ";
				$sql .= "FROM ";
				$sql .= "  accounts INNER JOIN accounts_brands ON accounts_brands.account_id = accounts.id AND accounts_brands.brand_id = '$focus_id'";
				$sql .= " WHERE ";
				$sql .= "  accounts.deleted=0 AND accounts_brands.deleted=0) ";
				$sql .= " UNION ALL ";
				$sql .= "(SELECT '' as reports_to_id,accounts.id,accounts.name, industry as tooltip, 'Accounts' as type, accounts.assigned_user_id, ' ' as acc_id,' ' as status ";
				$sql .= " FROM brands INNER JOIN brands_contacts ON (brands.id = brands_contacts.brand_id) AND brands.id='$focus_id' ";
				$sql .= " INNER JOIN accounts_contacts ON (brands_contacts.contact_id = accounts_contacts.contact_id ) ";
				$sql .= " INNER JOIN accounts ON accounts_contacts.account_id = accounts.id ";
				$sql .= " WHERE accounts.deleted=0 AND brands_contacts.deleted=0 AND ";
				$sql .= " accounts_contacts.deleted=0 AND brands.deleted=0 )";
			}
			elseif($type == 'Accounts')
			{
				$sql = "(SELECT ";
				$sql .= "  ' ' as reports_to_id, contacts.id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip, function_mast.name as function_name,'Contacts' as type, contacts.assigned_user_id, account_id as acc_id ";
				$sql .= "FROM ";
				$sql .= "  brands_contacts ";
				$sql .= "INNER JOIN ";
				$sql .= "  contacts ";
				$sql .= " ON ";
				$sql .= "  brands_contacts.contact_id = contacts.id AND brands_contacts.brand_id='$focus_id'";
				$sql .= " INNER JOIN accounts_contacts ON accounts_contacts.contact_id = contacts.id AND accounts_contacts.account_id = '$id' ";
				$sql .= " LEFT JOIN function_mast ON contacts.function_id = function_mast.id ";
				$sql .= " WHERE ";
				$sql .= "  accounts_contacts.deleted=0 AND brands_contacts.deleted=0 AND contacts.deleted=0) ";
//				$sql .= "UNION ALL ";
//				$sql .= "(SELECT ";
//				$sql .= "  '', id, name, industry,'' as function_name, 'Accounts', assigned_user_id, id ";
//				$sql .= "FROM ";
//				$sql .= "  accounts ";
//				$sql .= "WHERE ";
//				$sql .= "  deleted=0 AND parent_id = '$id') ";
			...
			{
				$sql  = "SELECT ";
				$sql .= "  reports_to_id, contacts.id as id, concat(ifnull(first_name, ''), ' ', ifnull(last_name, '')) as name, title as tooltip, function_mast.name as function_name, 'Contacts' as type, contacts.assigned_user_id, account_id as acc_id ";
				$sql .= " FROM ";
				$sql .= "  contacts LEFT JOIN function_mast ON contacts.function_id = function_mast.id ";
				$sql .= " INNER JOIN ";
...
//		$GLOBALS['log']->debug("SQL to execute :".$sql);
		$result = $db->query($sql);
...
//					if($type != 'Contacts')
				...
				if($circ_ref != 'Skip Done')
				{
					...
					*/					
					if(!isset($current_team))$current_team = new TeamOS();
//					$GLOBALS['log']->debug("Can Access Record ".$current_team);
					$owner = ($row['assigned_user_id'] == $current_user->id);
										
					if(($row['type'] == 'Brands') && $current_team->can_access_record($row['type'], 'view',$owner))
						$link .= "<a status='' title='" . strip_tags($row['tooltip']) . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name'];
					elseif($row['type'] == 'Contacts' && $current_team->can_access_record($row['type'], 'view',$owner))
						$link .= "<a status='' title='" . strip_tags($row['tooltip']) . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name'] . "<hr>" . $row['tooltip'] . "<hr>" . $row['function_name'];
					elseif($row['type'] == 'Accounts' && $current_team->can_access_record($row['type'], 'view',$owner))
						$link .= "<a status='' title='" . strip_tags($row['tooltip']) . "' style='text-decoration: none' href='' onClick=" . '"' . "JavaScript:window.opener.location='index.php?action=DetailView&module=" . $row['type'] . "&record=" . $row['id'] . "';window.close();return false;" . '"' . "'>$circ_ref " . $row['name'] . "<hr>" . $row['tooltip'];
					else
...
#debug					echo "\$row['id']=" . $row['id'] . " type=$type\n";
	...
	global $mod_strings;
	$title = $mod_strings['LBL_ORGCHART_TITLE']."-".$mod_strings['LBL_MODULE_NAME'];
	$first_entity = $mod_strings['LBL_ORGCHART_1ST_ENTITY'];
	$brand_entity = $mod_strings['LBL_ORGCHART_BRAND_ENTITY'];
	$primary_account_entity = $mod_strings['LBL_ORGCHART_PRIMARY_ACCOUNT_ENTITY'];	
	...
	$color_brand    = $colors['brand_entity'];	
	$color_primary_account    = $colors['primary_account_entity'];		
  ...
  <table cellSpacing=1 cellPadding=0 width="100%" style='border: 1px solid #dfdfdf;'>
	 <tr>
		<td>$description:&nbsp;&nbsp;</td>
		<td align=center width=120 style='color: gray; background-color: $color_primary_account'>$primary_account_entity</td>
		<td align=center width=120 style='color: gray; background-color: $color_first'>$first_entity</td>
		<td align=center width=120 style='color: gray; background-color: $color_brand'>$brand_entity</td>
...
EOQ;
		...
		switch ($type) {
			case "Accounts":
				...
				switch ($status) {
					...
					$color = $colors['primary_account_entity'];
				}
				break;
			case "Brands":
				$color = $colors['brand_entity'];
				break;
			case "Contacts":
	...
	function add_box($data, $indent, $first, $focused, $color='', $id)
	{
		...
		if(strpos($data, "restricted")===false)
			echo $indent."  <table align=center class='outline_box' width=150 cellspacing=0 cellpadding=0>"."\n";
		else {
			echo $indent."  <table align=center class='outline_box' width=150 cellspacing=0 cellpadding=0>"."\n";
			$id = 'na';
...
//			$color = $colors['current_record'];
		...
		echo $indent."      <td id='$id' class='$box_type' align=center style='background-color: $color; padding-left: 5px; padding-right: 5px; margin-left: 10px; margin-right: 10px; border: 1px solid #dfdfdf;' height=95>"."\n";
